...
# Pytest-style tests for PathBoundaryEnforcer that need tmp_path fixture
def test_validate_path_within_boundary(tmp_path):
...
def test_validate_path_outside_boundary(tmp_path):
...
def test_sanitize_path_relative(tmp_path):
...
def test_sanitize_path_with_dotdot(tmp_path):
...
def test_sanitize_path_absolute_within_boundary(tmp_path):
...
def test_sanitize_path_absolute_outside_boundary(tmp_path):
