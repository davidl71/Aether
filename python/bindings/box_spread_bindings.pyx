...
# distutils: sources = ../../src/box_spread_strategy.cpp ../../src/risk_calculator.cpp ../../src/option_chain.cpp
...
cdef class PyBoxSpreadStrategy:
    ...
    def calculate_arbitrage_profit(self, PyBoxSpreadLeg spread):
        ...
        if not self._strategy:
            raise RuntimeError("Strategy not initialized")
        return self._strategy.get().calculate_arbitrage_profit(spread._ptr[0])
...
def calculate_arbitrage_profit(PyBoxSpreadLeg spread):
    """Standalone function to calculate arbitrage profit"""
    return spread.arbitrage_profit
