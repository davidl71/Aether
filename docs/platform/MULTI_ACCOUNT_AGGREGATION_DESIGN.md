...
**Israeli bank scrapers** (service port 8010): The scrapers support multiple companies per [israeli-bank-scrapers](https://github.com/eshaham/israeli-bank-scrapers); each company is an independent source. Select via `SCRAPER_COMPANY_ID` or POST `/scrape` body `companyId`. Available companies include **fibi**, **max**, **visaCal**, **discount**, **leumi**, **hapoalim**, **isracard**, **beinleumi**, **mizrahi**, etc. You can run scrapes for different companies in parallel (e.g. different processes or sequential runs); each writes to the shared ledger under `Assets:Bank:{BankName}:{accountNumber}`. Config `broker.priorities` lists which providers are used for ordering/display: `["ib", "alpaca", "fibi", "meitav", "ibi", "discount", "mock"]` — all of these are independent. The scrapers service is currently disabled by default in the example config because the preferred workflow is manual file-based position import.
