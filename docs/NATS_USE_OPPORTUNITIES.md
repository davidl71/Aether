...
- **NATS → QuestDB (implemented):** The **Go nats-questdb-bridge** (`agents/go/cmd/nats-questdb-bridge`) subscribes to **Core NATS** `market-data.tick.>` (or JetStream `market.data.>`) and writes each message to QuestDB via ILP (table `market_data`). Run via `./scripts/run_questdb_nats_writer.sh` or `./scripts/service.sh start questdb_nats`. Requires Go. For protobuf ingestion from Rust, use an adapter that republishes proto→JSON to JetStream and run the bridge.
