...
- **Setup script:** Run `source ./scripts/setup_op_service_account.sh` to load the token from `OP_SERVICE_ACCOUNT_TOKEN` or from `~/.config/op/service_account_token` (or `$OP_SERVICE_ACCOUNT_TOKEN_FILE`) and export it into your shell. Use `./scripts/setup_op_service_account.sh verify` to test the connection. Optionally run the `op` CLI to create the token and secret items:
  ...
  - **`./scripts/setup_op_service_account.sh setup-secrets`** — Sign in, then create placeholder 1Password items (Alpaca API, Tastytrade, TradeStation API, FRED API) in a vault and print suggested `OP_*_SECRET` exports. Set `OP_SETUP_VAULT` or enter the vault name when prompted.
...
- **Config missing:** When you run the TUI with `./scripts/run_python_tui.sh` and the shared config file (`config/config.json`) does not exist, the script detects an interactive terminal and runs the full 1Password setup (`setup-full`) before starting the TUI. After setup, it sources the token into the shell and shows a tip to copy `config/config.example.json` to `config/config.json` and add `op://` refs. Non-interactive runs (e.g. CI or piped) skip this and start the TUI with defaults.
...
| **TradeStation** | `OP_TRADESTATION_CLIENT_ID_SECRET`, `OP_TRADESTATION_CLIENT_SECRET_SECRET` | Vault / TradeStation API / client_id, client_secret |
| | `OP_TRADESTATION_ACCOUNT_ID_SECRET` | same or separate item / account_id |
