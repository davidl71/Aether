...
- **NAUTILUS_IMPLEMENTATION_SUMMARY** (doc removed)
...
- **CODE_VERIFICATION_REVIEW** (doc removed)
- **CODE_IMPROVEMENTS_FROM_NOTEBOOKLM** (doc removed)
...
- **EXTENSION_AUDIT_RESULTS** (doc removed)
- **EXTENSION_REDUNDANCY_REPORT** (doc removed)
...
- **TODO2_PRIORITY_ALIGNMENT_ANALYSIS** (doc removed)
- **TODO2_SYNTHETIC_FINANCING_ALIGNMENT_ANALYSIS** (doc removed)
...
- **indices/FIX_PROTOCOL_INDEX** (doc removed)
...
Deprecated or superseded documents were previously under `docs/archive/`; that tree has been removed. For current runtime and backend ownership, see [ARCHITECTURE.md](../ARCHITECTURE.md) and [platform/CURRENT_TOPOLOGY.md](platform/CURRENT_TOPOLOGY.md).
...
- **external/CPP20_FEATURES** (doc removed)
