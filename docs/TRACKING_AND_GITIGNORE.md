...
| **Project source** | **Track** | Rust in `agents/backend/`, optional Python (`agents/nautilus/`, `scripts/*.py`); see **`docs/PYTHON_INVENTORY.md`**. Docs, scripts, Ansible, shared editor config. Legacy `native/` references are historical only. |
...
- **`python/generated/`** – Legacy ignore path (local betterproto output if you still generate it); root `./proto/generate.sh` does not emit Python.
