...
- **Execute:** For each task ID, run one subagent (e.g. Cursor `mcp_task` or “wave-task-runner”) with:
  ...
  - **Context** = plan file [.cursor/plans/ib_box_spread_full_universal.plan.md](../../.cursor/plans/ib_box_spread_full_universal.plan.md) and the task description from Todo2 or the plan.
