...
Rebuild with `ninja -C build`. Run the CLI/TUI via `./build/ib_box_spread`; it emits tab-separated summaries for three strike widths. Open the macOS bundle using `open build/ib_box_spread_app.app`. Produce a universal binary when needed with `./build_universal.sh <cppclient-dir> <libTwsApiCpp.dylib>`. Execute regression tests after every change: `ctest --test-dir build --output-on-failure`.
