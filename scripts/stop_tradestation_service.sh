...
if lsof -ti :${TRADESTATION_PORT} >/dev/null 2>&1; then
  ...
  echo "[info] Stopping TradeStation service on port ${TRADESTATION_PORT} (PID: $PID)..."
  ...
  echo "[info] TradeStation service stopped"
else
  echo "[info] TradeStation service not running on port ${TRADESTATION_PORT}"
fi
