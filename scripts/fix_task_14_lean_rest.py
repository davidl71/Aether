#!/usr/bin/env python3
"""Fix path issues in LEAN_REST_API_WRAPPER_DESIGN.md (Task 14)"""
...
def fix_lean_rest_links():
    """Fix broken links in LEAN_REST_API_WRAPPER_DESIGN.md"""
    docs_dir = Path('/home/david/ib_box_spread_full_universal/docs')
    root_dir = Path('/home/david/ib_box_spread_full_universal')
    md_file = docs_dir / 'research' / 'integration' / 'LEAN_REST_API_WRAPPER_DESIGN.md'
    ...
    try:
        ...
        # Fix ./agents/shared/API_CONTRACT.md (line 870)
        target_line_num = 870
        if target_line_num <= len(lines):
            line = lines[target_line_num - 1]
            if './agents/shared/API_CONTRACT.md' in line:
                ...
                if found_file and found_file.exists():
                    old_link = './agents/shared/API_CONTRACT.md'
...
if __name__ == '__main__':
    result = fix_lean_rest_links()
    report_file = Path('/home/david/ib_box_spread_full_universal/docs/TASK_14_FIX_REPORT.json')
    report_file.write_text(json.dumps(result, indent=2), encoding='utf-8')
    print(f"Task 14: {'✅' if result['success'] else '❌'} {len(result.get('fixes', []))} fixes")
