...
# Prefers Go nats-questdb-bridge (single binary); falls back to Python when Go unavailable.
...
# Prefer Go bridge (Core NATS subject market-data.tick.> to match Python behavior)
...
# Fallback: Python writer
export PYTHONPATH="${ROOT_DIR}/python:${PYTHONPATH:-}"
exec python3 -m integration.questdb_nats_writer "$@"
