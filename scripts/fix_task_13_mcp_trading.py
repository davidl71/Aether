#!/usr/bin/env python3
"""Fix path issues in MCP_TRADING_SERVER_COMPLETE.md (Task 13)"""
...
def find_file_by_path(target_path: str, base_file: Path, root_dir: Path) -> tuple[Path | None, str | None]:
    ...
    if target_path.startswith('./'):
        target_file = (base_dir / target_path[2:]).resolve()
...
def fix_mcp_trading_links():
    """Fix broken links in MCP_TRADING_SERVER_COMPLETE.md"""
    docs_dir = Path('/home/david/ib_box_spread_full_universal/docs')
    root_dir = Path('/home/david/ib_box_spread_full_universal')
    md_file = docs_dir / 'MCP_TRADING_SERVER_COMPLETE.md'
    ...
    try:
        ...
        # Fix ./mcp/trading_server/CYTHON_BINDINGS_GUIDE.md (line 425)
        target_line_num = 425
        if target_line_num <= len(lines):
            line = lines[target_line_num - 1]
            if './mcp/trading_server/CYTHON_BINDINGS_GUIDE.md' in line:
                found_file, rel_path = find_file_by_path(
                    './mcp/trading_server/CYTHON_BINDINGS_GUIDE.md',
                    md_file,
                    root_dir
                )
                
                if found_file and found_file.exists():
                    old_link = './mcp/trading_server/CYTHON_BINDINGS_GUIDE.md'
...
if __name__ == '__main__':
    result = fix_mcp_trading_links()
    report_file = Path('/home/david/ib_box_spread_full_universal/docs/TASK_13_FIX_REPORT.json')
    report_file.write_text(json.dumps(result, indent=2), encoding='utf-8')
    print(f"Task 13: {'✅' if result['success'] else '❌'} {len(result.get('fixes', []))} fixes")
