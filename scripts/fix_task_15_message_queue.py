#!/usr/bin/env python3
"""Fix path issues in MESSAGE_QUEUE_ARCHITECTURE.md (Task 15)"""
...
def fix_message_queue_links():
    """Fix broken links in MESSAGE_QUEUE_ARCHITECTURE.md"""
    docs_dir = Path('/home/david/ib_box_spread_full_universal/docs')
    root_dir = Path('/home/david/ib_box_spread_full_universal')
    md_file = docs_dir / 'research' / 'architecture' / 'MESSAGE_QUEUE_ARCHITECTURE.md'
    ...
    try:
        ...
        # Fix ./COMPONENT_COORDINATION_ANALYSIS.md (line 658)
        target_line_num = 658
        if target_line_num <= len(lines):
            line = lines[target_line_num - 1]
            if './COMPONENT_COORDINATION_ANALYSIS.md' in line:
                found_file, rel_path = find_file_by_path(
                    './COMPONENT_COORDINATION_ANALYSIS.md',
                    md_file,
                    root_dir
                )
                
                if found_file and found_file.exists():
                    old_link = './COMPONENT_COORDINATION_ANALYSIS.md'
...
if __name__ == '__main__':
    result = fix_message_queue_links()
    report_file = Path('/home/david/ib_box_spread_full_universal/docs/TASK_15_FIX_REPORT.json')
    report_file.write_text(json.dumps(result, indent=2), encoding='utf-8')
    print(f"Task 15: {'✅' if result['success'] else '❌'} {len(result.get('fixes', []))} fixes")
