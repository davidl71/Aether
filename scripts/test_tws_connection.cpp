...
int main(int argc, char* argv[]) {
    ...
    int port = 4002;  // Default to IB Gateway Paper Trading (4002) instead of TWS Paper (7497)
    ...
    client->set_error_callback([](int code, const std::string& msg) {
        std::cerr << "[ERROR CALLBACK] Code: " << code << " | Message: " << msg << std::endl;
    });
    ...
    if (connected) {
        std::cout << "✓ SUCCESS: Connected to TWS in " << connect_duration << "ms" << std::endl;
        std::cout << std::endl;
        ...
        std::cout << "Connection is stable. Keeping connection alive for 5 seconds..." << std::endl;
        for (int i = 0; i < 5; ++i) {
        ...
        std::cerr << "✗ FAILED: Could not connect to TWS after " << connect_duration << "ms" << std::endl;
        std::cerr << std::endl;
        std::cerr << "Troubleshooting Steps:" << std::endl;
        std::cerr << "  1. Verify TWS/IB Gateway is running" << std::endl;
        std::cerr << "     → Check if process is running: ps aux | grep -i tws" << std::endl;
        std::cerr << "     → Check if port is listening: lsof -i :" << port << std::endl;
        std::cerr << std::endl;
        std::cerr << "  2. Verify API is enabled in TWS/Gateway" << std::endl;
        std::cerr << "     → Go to: File → Global Configuration → API → Settings" << std::endl;
        std::cerr << "     → Enable: 'Enable ActiveX and Socket Clients'" << std::endl;
        std::cerr << "     → Verify port matches: " << port << std::endl;
        std::cerr << std::endl;
        std::cerr << "  3. Check TWS/Gateway window for connection prompts" << std::endl;
        std::cerr << "     → Look for 'A new API client is attempting to connect' message" << std::endl;
        std::cerr << "     → Click 'Accept' or 'OK' to allow the connection" << std::endl;
        std::cerr << std::endl;
        std::cerr << "  4. Verify client ID is unique" << std::endl;
        std::cerr << "     → Current client ID: " << client_id << std::endl;
        std::cerr << "     → Try a different client ID if another application is using " << client_id << std::endl;
        std::cerr << std::endl;
