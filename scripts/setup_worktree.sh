...
build_intel_decimal() {
  ...
  local intel_dir="${WORKTREE_DIR}/native/third_party/IntelRDFPMathLib20U4/LIBRARY"
  ...
  if [[ ! -d "${intel_dir}" ]]; then
    ...
    log_error "Please ensure the library is extracted to native/third_party/IntelRDFPMathLib20U4/"
    exit 1
  fi
