...
public:
    ...
    void position(const std::string& account, const Contract& contract,
        ...
        for (auto& p : positions_) {
            if (p.contract.symbol == pos.contract.symbol &&
                p.contract.expiry == pos.contract.expiry &&
                p.contract.strike == pos.contract.strike &&
                p.contract.type == pos.contract.type) {
                p = pos;
                found = true;
                break;
            }
    ...
    void updatePortfolio(const Contract& contract, Decimal position,
        ...
        for (auto& p : positions_) {
            if (p.contract.symbol == contract.symbol &&
                p.contract.expiry == contract.lastTradeDateOrContractMonth &&
                std::abs(p.contract.strike - contract.strike) < 0.01) {
                p.current_price = marketPrice;
                break;
            }
        }
    }
