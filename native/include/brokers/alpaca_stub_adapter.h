...
/**
 * Alpaca Stub Adapter - Implements IBroker interface with mock data
 *
 ...
 * real Alpaca API integration. Returns realistic mock data that matches
 * the expected data structures.
 */
class AlpacaStubAdapter : public box_spread::brokers::IBroker {
public:
    /**
     * Configuration for Alpaca stub adapter
     */
    struct Config {
        std::string api_key_id = "STUB_KEY";
        std::string api_secret_key = "STUB_SECRET";
    ...
    /**
     * Constructor
     * @param config Alpaca stub configuration
     */
    explicit AlpacaStubAdapter(const Config& config);

    /**
     * Destructor
     */
    ~AlpacaStubAdapter() override;

    // Disable copy
    AlpacaStubAdapter(const AlpacaStubAdapter&) = delete;
    AlpacaStubAdapter& operator=(const AlpacaStubAdapter&) = delete;
