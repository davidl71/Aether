...
int main(int argc, char* argv[]) {
    ...
    std::cout << "╔══════════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║       IBKR Packet-Level Diagnostic Trace                        ║" << std::endl;
    std::cout << "╚══════════════════════════════════════════════════════════════════╝" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "╔══════════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║  Phase 1: Pre-Connection Checks                                 ║" << std::endl;
    std::cout << "╚══════════════════════════════════════════════════════════════════╝" << std::endl;
    std::cout << std::endl;
    ...
    if (result < 0 && errno == EINPROGRESS) {
        ...
        if (select_result > 0) {
            ...
            if (error == 0) {
                ...
                std::cout << "✓ IB Gateway is listening and accepting connections" << std::endl;
            } else {
                std::cout << "✗ Connection failed with error: " << strerror(error) << std::endl;
            }
        } else {
        ...
        std::cout << "✗ Cannot reach port " << port << ": " << strerror(errno) << std::endl;
    }
    ...
    std::cout << "╔══════════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║  Phase 2: TWS API Connection with Detailed Tracing              ║" << std::endl;
    std::cout << "╚══════════════════════════════════════════════════════════════════╝" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" << std::endl;
    std::cout << "Starting connection (watch for callback sequence)..." << std::endl;
    std::cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" << std::endl;
    std::cout << std::endl;
    ...
    if (!connected) {
        ...
        std::cout << "\nThis means the connection was rejected during handshake." << std::endl;
        std::cout << "Check the error messages above for specific details." << std::endl;
        return 1;
    }
    
    std::cout << "\n✓ TWSClient.connect() returned true after " 
             << api_connect_ms << "ms" << std::endl;
    std::cout << std::endl;
    
    std::cout << "╔══════════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║  Phase 3: Callback Monitoring (60 seconds)                      ║" << std::endl;
    std::cout << "╚══════════════════════════════════════════════════════════════════╝" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << std::endl;
    std::cout << "Watching for 60 seconds (or until disconnection)..." << std::endl;
    std::cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" << std::endl;
    std::cout << std::endl;
    ...
    while (iterations < max_iterations && still_connected) {
        ...
        if (iterations % 10 == 0) {
            ...
            // The TWSClient doesn't expose isConnected(), but we can infer from behavior
            // If we haven't seen any errors, assume still connected
            std::cout << "[" << std::setw(2) << elapsed_sec << "s] Monitoring..." << std::endl;
        }
        
        // Stop if we've gotten what we need (would need to track callbacks properly)
        // For now, just monitor for the full duration or until manual stop
    }
    ...
    std::cout << "╔══════════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║  Phase 4: Analysis and Recommendations                          ║" << std::endl;
    std::cout << "╚══════════════════════════════════════════════════════════════════╝" << std::endl;
    std::cout << std::endl;
    
    std::cout << "Review the log output above to see:" << std::endl;
    std::cout << "  • Which callbacks were received" << std::endl;
    std::cout << "  • When 'Connection closed by TWS' appeared (if at all)" << std::endl;
    ...
    std::cout << "    ✗ Connection closed by TWS (immediately after)" << std::endl;
    std::cout << "    ✗ managedAccounts never received" << std::endl;
    std::cout << std::endl;
    std::cout << "  → Gateway is REJECTING the connection" << std::endl;
    std::cout << "  → Enable 'Accept incoming connection requests automatically'" << std::endl;
    std::cout << "  → In: IB Gateway → Configure → Settings → API → Settings" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "  2. Follow the recommendations in FIX_IBKR_CONNECTION.md" << std::endl;
    std::cout << "  3. After fixing settings, run: ./scripts/diagnose_ibkr.sh" << std::endl;
    std::cout << std::endl;
