// test_diagnostic_connect.cpp - Deep diagnostic connection test with callback
// tracing Shows exactly what callbacks are being received and when
...
public:
  ...
  void log_callback(const std::string &msg) {
    ...
    std::string full_msg =
        std::string(timestamp) + "." + std::to_string(ms) + " - " + msg;
  ...
  void print_summary() {
    ...
    std::cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
              << std::endl;
    std::cout << "Total callbacks received: " << callback_log.size()
              << std::endl;
    std::cout << std::endl;
    std::cout << "Status:" << std::endl;
    std::cout << "  connectAck:      " << (connect_ack_received ? "✓" : "✗")
              << std::endl;
    std::cout << "  managedAccounts: "
              << (managed_accounts_received ? "✓" : "✗") << std::endl;
    std::cout << "  nextValidId:     " << (next_valid_id_received ? "✓" : "✗")
              << std::endl;
    std::cout << "  errors:          " << (error_received ? "✓" : "✗")
              << std::endl;
    std::cout << "  disconnected:    " << (connection_closed ? "✓" : "✗")
              << std::endl;
    std::cout << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  if (!connected) {
    std::cout << "\n✗ Connection failed after " << connect_duration << "ms"
              << std::endl;
    std::cout << "\nPossible causes:" << std::endl;
    std::cout << "  1. IB Gateway not running" << std::endl;
    std::cout << "  2. Wrong port (check 4001 vs 4002 vs 7496 vs 7497)"
              << std::endl;
    std::cout << "  3. API not enabled in Gateway settings" << std::endl;
    return 1;
  }

  std::cout << "✓ Socket connected in " << connect_duration << "ms"
            << std::endl;
  ...
  std::cout << "(Looking for: connectAck → managedAccounts → nextValidId)"
            << std::endl;
  std::cout << std::endl;
  ...
  for (int i = 0; i < total_iterations; ++i) {
    ...
    if (tracker.connect_ack_received && tracker.managed_accounts_received &&
        tracker.next_valid_id_received) {
      std::cout << "\n✓ All callbacks received! Connection fully established."
                << std::endl;
      break;
    }
  ...
  if (!tracker.connect_ack_received) {
    ...
    std::cout << "  3. Gateway rejected connection before handshake"
              << std::endl;

  } else if (!tracker.managed_accounts_received && !tracker.connection_closed) {
    std::cout
        << "⚠️  WARNING: Got connectAck but no managedAccounts (still connected)"
        << std::endl;
    std::cout << std::endl;
    std::cout << "Gateway is waiting for something. This usually means:"
              << std::endl;
    std::cout << "  1. Connection approval dialog waiting (not visible?)"
              << std::endl;
    std::cout << "  2. 'Accept incoming requests automatically' is OFF"
              << std::endl;
    std::cout << "  3. Security restrictions blocking the connection"
              << std::endl;
    std::cout << std::endl;
    std::cout << "Check IB Gateway window for any prompts or notifications!"
              << std::endl;

  } else if (!tracker.managed_accounts_received && tracker.connection_closed) {
    std::cout << "❌ REJECTED: Gateway closed connection after connectAck"
              << std::endl;
    std::cout << std::endl;
    std::cout << "The connection was explicitly rejected by IB Gateway."
              << std::endl;
    std::cout << "Most common cause:" << std::endl;
    std::cout
        << "  → 'Accept incoming connection requests automatically' is DISABLED"
        << std::endl;
    std::cout << std::endl;
  ...
  } else if (tracker.managed_accounts_received &&
             !tracker.next_valid_id_received) {
    std::cout << "⚠️  PARTIAL: Got managedAccounts but no nextValidId"
              << std::endl;
    std::cout << std::endl;
    std::cout
        << "Connection partially established but not ready for operations."
        << std::endl;
    std::cout << "This is unusual - may indicate:" << std::endl;
    std::cout << "  1. Gateway is still initializing" << std::endl;
    std::cout << "  2. Read-only API mode preventing order ID assignment"
              << std::endl;
  ...
  if (tracker.next_valid_id_received) {
    ...
    std::cout << "\nResult: Found " << positions.size() << " position(s)"
              << std::endl;
