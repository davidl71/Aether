...
TEST_CASE("MarketHours - Early close detection",
          "[market_hours][early_close]") {
  MarketHours mh;
...
TEST_CASE("MarketHours - Market status during regular hours",
          "[market_hours][status]") {
  MarketHours mh;
...
TEST_CASE("MarketHours - Market status on holiday",
          "[market_hours][holiday_status]") {
  MarketHours mh;
...
TEST_CASE("MarketHours - DST timezone conversion", "[market_hours][dst]") {
  ...
  // Market should be open during regular hours (2:00 PM ET = 6:00 PM UTC during
  // DST) Actually: 4:00 PM UTC - 4 hours (EDT) = 12:00 PM ET (regular hours)
