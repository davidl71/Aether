...
TEST_CASE("PathValidator - Traversal detection", "[security][path_validator]") {
  SECTION("Detects ../ pattern") {
    REQUIRE(PathValidator::contains_traversal(
        std::filesystem::path("../test.txt")));
  }

  SECTION("Detects ..\\ pattern") {
    REQUIRE(PathValidator::contains_traversal(
        std::filesystem::path("..\\test.txt")));
  }

  SECTION("Detects encoded traversal") {
    REQUIRE(PathValidator::contains_traversal(
        std::filesystem::path("test%2e%2e%2foutside.txt")));
  }

  SECTION("Does not detect false positives") {
    REQUIRE_FALSE(
        PathValidator::contains_traversal(std::filesystem::path("test.txt")));
    REQUIRE_FALSE(PathValidator::contains_traversal(
        std::filesystem::path("dir/test.txt")));
  }
}
...
TEST_CASE("PathValidator - Default validator", "[security][path_validator]") {
  ...
  SECTION("Allows home directory") {
    ...
    if (home) {
      auto result =
          validator.validate_path(std::filesystem::path(home) / ".config" /
                                  "ib_box_spread" / "test.json");
