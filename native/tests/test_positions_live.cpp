...
int main(int argc, char **argv) {
  ...
  if (tws_config.port != 7497) {
    ...
    std::cout << "⚠️  Switching to paper trading port 7497 for safety"
              << std::endl;
  ...
  if (!client.connect()) {
    spdlog::error(
        "Failed to connect to TWS. Is TWS/Gateway running on port {}?",
        tws_config.port);
    std::cout << std::endl;
    ...
    std::cout
        ...
        << std::endl;
    std::cout << "   3. Check the API port matches (paper=7497, live=7496)"
              << std::endl;
  ...
  if (positions.empty()) {
    ...
    std::cout << std::endl;
    std::cout << "💡 This is expected if your paper account has no positions."
              << std::endl;
    std::cout
        << "   To test with positions, place some test trades in TWS first."
        << std::endl;
  } else {
    ...
    for (const auto &pos : positions) {
      ...
      std::cout << "Type:     "
                << (pos.contract.type == types::OptionType::Call ? "CALL"
                                                                 : "PUT")
                << std::endl;
      ...
      double market_value = pos.quantity * pos.current_price *
                            100.0; // Options are per 100 shares
  ...
  if (account_info) {
    std::cout << "Account ID:         " << account_info->account_id
              << std::endl;
    std::cout << "Net Liquidation:    $" << account_info->net_liquidation
              << std::endl;
    std::cout << "Cash Balance:       $" << account_info->cash_balance
              << std::endl;
    std::cout << "Buying Power:       $" << account_info->buying_power
              << std::endl;
    std::cout << "Gross Position Val: $" << account_info->gross_position_value
              << std::endl;
    std::cout << "Unrealized P&L:     $" << account_info->unrealized_pnl
              << std::endl;
    std::cout << "Realized P&L:       $" << account_info->realized_pnl
              << std::endl;
  } else {
