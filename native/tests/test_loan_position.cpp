...
TEST_CASE("LoanPosition validation", "[loan]") {
    SECTION("Valid SHIR-based loan passes validation") {
        ...
        loan.maturity_date = loan.origination_date + std::chrono::hours(24 * 365 * 10);  // 10 years
        loan.next_payment_date = loan.origination_date + std::chrono::hours(24 * 30);  // 1 month
    ...
    SECTION("Valid CPI-linked loan passes validation") {
        ...
        loan.origination_date = std::chrono::system_clock::now();
        loan.maturity_date = loan.origination_date + std::chrono::hours(24 * 365 * 10);
        loan.next_payment_date = loan.origination_date + std::chrono::hours(24 * 30);
    ...
    SECTION("Invalid date order fails validation") {
        ...
        loan.payment_frequency_months = 1;
        loan.origination_date = std::chrono::system_clock::now();
        loan.maturity_date = loan.origination_date - std::chrono::hours(24 * 365);  // Before origination
...
TEST_CASE("LoanPosition calculations", "[loan]") {
    ...
    SECTION("Days until next payment") {
        ...
        loan.next_payment_date = now + std::chrono::hours(24 * 7);  // 7 days from now
    ...
    SECTION("Overdue detection") {
        LoanPosition loan;
        loan.status = LoanStatus::ACTIVE;
        loan.next_payment_date = std::chrono::system_clock::now() - std::chrono::hours(24 * 2);  // 2 days ago
...
TEST_CASE("LoanManager calculations", "[loan]") {
    ...
    loan1.maturity_date = loan1.origination_date + std::chrono::hours(24 * 365 * 10);
    loan1.next_payment_date = loan1.origination_date + std::chrono::hours(24 * 30);
    ...
    loan2.maturity_date = loan2.origination_date + std::chrono::hours(24 * 365 * 10);
    loan2.next_payment_date = loan2.origination_date + std::chrono::hours(24 * 30);
...
TEST_CASE("LoanManager CPI and SHIR updates", "[loan]") {
    ...
    cpi_loan.maturity_date = cpi_loan.origination_date + std::chrono::hours(24 * 365 * 10);
    cpi_loan.next_payment_date = cpi_loan.origination_date + std::chrono::hours(24 * 30);
