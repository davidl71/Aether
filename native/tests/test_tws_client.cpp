...
bool is_port_listening(const std::string &host, int port,
                       int timeout_ms = 500) {
  ...
  // Attempt connection
  int result = connect(sock, reinterpret_cast<struct sockaddr *>(&server_addr),
                       sizeof(server_addr));
...
TEST_CASE("TWS Client - Port detection (integration)",
          "[tws][connectivity][integration]") {
...
TEST_CASE("TWS Client - Connection attempt (integration)",
          "[tws][connectivity][integration]") {
  ...
  if (!test_done) {
    ...
    while (!test_done && (std::chrono::steady_clock::now() - stop_start) <
  ...
  if (connected) {
    ...
    INFO("Connection failed (TWS may require authentication or have API "
         "disabled)");
...
TEST_CASE("TWS Client - Paper/Live mismatch detection", "[tws][mismatch]") {
  ...
  // This is a unit test that verifies the logic without requiring actual
  // connections
...
TEST_CASE("TWS Client - Port fallback logic (integration)",
          "[tws][connectivity][integration]") {
  // Test that the client can find the correct port when configured port is
  // wrong
...
TEST_CASE("TWS Client - Market hours check (mock mode)",
          "[tws][market_hours]") {
  config::TWSConfig config;
  config.host = "127.0.0.1";
  config.port = 7497;
...
TEST_CASE("TWS Client - Option chain request (mock mode)",
          "[tws][option_chain]") {
  config::TWSConfig config;
  config.host = "127.0.0.1";
  config.port = 7497;
