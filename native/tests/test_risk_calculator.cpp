...
TEST_CASE("RiskCalculator portfolio risk calculations", "[risk]") {
    ...
    SECTION("Calculate total exposure") {
        auto portfolio_risk = calculator.calculate_portfolio_risk(positions, account);

        REQUIRE(portfolio_risk.total_exposure > 0);
    }

    SECTION("Check remaining capacity") {
        double remaining = calculator.calculate_remaining_capacity(positions, account.net_liquidation);
...
TEST_CASE("RiskCalculator position sizing", "[risk]") {
    ...
    SECTION("Fixed fractional sizing") {
        int fixed_size = calculator.calculate_fixed_fractional_size(
            250.0,    // Position cost
...
TEST_CASE("Risk-adjusted return calculations", "[risk]") {
    ...
    SECTION("Calculate Sortino ratio") {
        double sortino = calculator.calculate_sortino_ratio(returns, risk_free_rate);
...
TEST_CASE("VaR calculations", "[risk]") {
    ...
    SECTION("Parametric VaR") {
        double var_param = calculator.calculate_var_parametric(
            0.01,      // Expected return
            0.15,      // Volatility
...
TEST_CASE("Helper functions", "[risk]") {
    ...
    SECTION("Calculate correlation") {
        ...
        REQUIRE_THAT(corr, Catch::Matchers::WithinRel(1.0, 0.001));  // Perfect positive correlation
    }
    ...
    SECTION("Annualize volatility") {
        double annualized_vol = annualize_volatility(0.02, 252);  // 2% daily volatility
