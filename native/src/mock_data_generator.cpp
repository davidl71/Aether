...
types::BoxSpreadBag MockBoxSpreadBagGenerator::generate_bag(
    ...
    if (underlying_symbol == "ES" || underlying_symbol.find("ES") != std::string::npos) {
    ...
    types::OptionStyle style = (underlying_symbol == "SPX" || underlying_symbol == "XSP")
                               ? types::OptionStyle::European
                               : types::OptionStyle::American;

    spread.long_call = types::OptionContract{
        underlying_symbol, expiry, strike_low, types::OptionType::Call, style, "CBOE", ""
    };
    spread.short_call = types::OptionContract{
        underlying_symbol, expiry, strike_high, types::OptionType::Call, style, "CBOE", ""
    };
    spread.long_put = types::OptionContract{
        underlying_symbol, expiry, strike_high, types::OptionType::Put, style, "CBOE", ""
    };
    spread.short_put = types::OptionContract{
        underlying_symbol, expiry, strike_low, types::OptionType::Put, style, "CBOE", ""
    };

    // Generate realistic net debit based on implied rate
    double implied_rate_percent = 5.0 + (rng_() % 200) / 10.0;  // 5.0% - 25.0%
    spread.net_debit = generate_realistic_net_debit(strike_width, days_to_expiry, implied_rate_percent);
    spread.theoretical_value = strike_width;
    spread.arbitrage_profit = spread.theoretical_value - spread.net_debit;
    spread.roi_percent = spread.net_debit > 0 ?
        (spread.arbitrage_profit / spread.net_debit) * 100.0 : 0.0;
    ...
    // Generate prices (simplified - would use Black-Scholes in full implementation)
    ...
    spread.long_call_bid_ask_spread = 0.05 + (rng_() % 50) / 1000.0;  // $0.05 - $0.10
    ...
    std::string actual_expiry = expiry.empty() ? generate_expiry_date(days_to_expiry) : expiry;
...
std::vector<types::BoxSpreadBag> MockBoxSpreadBagGenerator::generate_bags(
    size_t count,
    const std::string& underlying_symbol) {
...
void MockBoxSpreadBagGenerator::simulate_price_movement(
    ...
    for (int i = 0; i < num_updates; ++i) {
        // Simulate price movement (random walk with slight mean reversion to theoretical)
        ...
        types::BoxSpreadBagManager::update_bag_market_data(
            bag, bid, ask, current_price, 10, 10
        );
...
void MockBoxSpreadBagGenerator::generate_candle_history(
    ...
    for (int i = 0; i < num_candles; ++i) {
        ...
        candle.entry = bag.position.entry_price > 0 ? bag.position.entry_price : base_price;
...
void MockBoxSpreadBagGenerator::update_bag_market_data(
    ...
    types::BoxSpreadBagManager::update_bag_market_data(
        bag, bid, ask, mid_price, bid_size, ask_size
    );
...
std::vector<types::BoxSpreadBag> MockBoxSpreadBagGenerator::generate_multi_symbol_bags(
...
std::vector<types::BoxSpreadBag> MockBoxSpreadBagGenerator::generate_yield_curve_bags(
...
std::string MockBoxSpreadBagGenerator::generate_expiry_date(int days_to_expiry) {
    auto now = std::chrono::system_clock::now();
...
double MockBoxSpreadBagGenerator::add_realistic_noise(
    double value,
    double std_dev_percent) {

    std::normal_distribution<double> noise_dist(0.0, value * std_dev_percent / 100.0);
    return value + noise_dist(rng_);
}

void MockBoxSpreadBagGenerator::update_greeks_for_bag(
    types::BoxSpreadBag& bag,
    double underlying_price) {
    ...
    types::BoxSpreadBagManager::update_bag_greeks(
        bag, underlying_price, volatility, risk_free_rate
    );
}
...
std::string CboeSymbolFormatter::format_complex_symbol(
    ...
    return types::BoxSpreadBag::generate_cboe_symbol(underlying, expiry, strike_low, strike_high);
}

bool CboeSymbolFormatter::parse_complex_symbol(
