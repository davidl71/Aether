...
double InterestRateFuture::calculate_hedge_ratio(
    const types::BoxSpreadLeg& box_spread,
    double box_spread_notional) const {

    // Calculate box spread implied rate
    double box_spread_rate = strategy::BoxSpreadCalculator::calculate_implied_interest_rate(box_spread);
...
HedgeManager::HedgeManager() {
    spdlog::debug("HedgeManager created");
}

HedgeManager::~HedgeManager() {
    spdlog::debug("HedgeManager destroyed");
}

std::optional<InterestRateFuture> HedgeManager::find_rate_future_hedge(
    const types::BoxSpreadLeg& box_spread,
...
HedgeManager::RateHedgeCalculation HedgeManager::calculate_rate_hedge(
    ...
    calc.basis_risk_bps = (box_spread_rate - futures_rate) * 10000.0;  // Convert to basis points

    // Calculate number of contracts needed
    // Contract size is $1M for SOFR/ED, adjust for hedge ratio
    double contracts_raw = (box_spread_notional * target_hedge_ratio) / future.contract_size;
    ...
    double commission = 2.0 * 2.00;  // Round-trip commission (futures ~$2/contract)
    calc.hedge_cost = (spread_cost + commission) * calc.contracts_needed;

    // Hedge is viable if basis risk and cost are acceptable
    calc.is_valid = (std::abs(calc.basis_risk_bps) < 50.0) &&  // Basis risk < 50 bps
                    (calc.hedge_cost < box_spread_notional * 0.001);  // Cost < 0.1% of notional

    spdlog::debug("Rate hedge calculation: {} contracts, basis risk: {:.1f} bps, cost: ${:.2f}",
...
CurrencyHedge HedgeManager::calculate_currency_hedge(
    ...
    spdlog::debug("Currency hedge: {} {} -> {} {} (rate: {:.4f})",
                 exposure_amount, base_currency,
                 hedge.hedge_amount, hedge_currency,
                 hedge.current_rate);
...
double HedgeManager::get_exchange_rate(
    ...
    spdlog::warn("Exchange rate not available for {}/{} (using 1.0)", base_currency, hedge_currency);
    return 1.0;
}

double HedgeManager::calculate_currency_hedge_cost(const CurrencyHedge& hedge) const {
    return hedge.calculate_hedge_cost();
}

HedgeManager::CompleteHedge HedgeManager::calculate_complete_hedge(
    const types::BoxSpreadLeg& box_spread,
    double box_spread_notional,
    ...
    if (strategy.hedge_interest_rate) {
        auto future_opt = find_rate_future_hedge(box_spread, strategy.rate_future_type);
        if (future_opt.has_value()) {
            complete.rate_hedge = calculate_rate_hedge(
                box_spread, box_spread_notional, future_opt.value(), strategy.target_hedge_ratio);
    ...
    if (strategy.hedge_currency && !strategy.hedge_currency_code.empty()) {
        ...
        double currency_cost = calculate_currency_hedge_cost(complete.currency_hedge);
        complete.total_hedge_cost += currency_cost;
    }

    // Calculate total cost as basis points
    if (box_spread_notional > 0) {
        complete.total_hedge_cost_bps = (complete.total_hedge_cost / box_spread_notional) * 10000.0;
    }

    // Check if hedge is viable (cost within threshold)
    complete.is_viable = complete.total_hedge_cost_bps <= strategy.max_hedge_cost_bps;

    spdlog::info("Complete hedge calculated: cost: ${:.2f} ({:.1f} bps), viable: {}",
                complete.total_hedge_cost, complete.total_hedge_cost_bps, complete.is_viable);

    return complete;
}

HedgeManager::HedgeEffectiveness HedgeManager::monitor_hedge(
    ...
    effectiveness.current_hedge_ratio = hedge.hedge_ratio;  // Would calculate from current positions
    ...
    // Calculate drift
    effectiveness.hedge_drift_bps = std::abs(effectiveness.current_hedge_ratio - effectiveness.target_hedge_ratio) * 10000.0;

    // Check if rebalancing needed (if enabled in strategy)
    // For now, simple threshold check
    effectiveness.needs_rebalance = effectiveness.hedge_drift_bps > 10.0;  // 10 bps threshold

    if (effectiveness.needs_rebalance) {
        // Estimate rebalancing cost
        effectiveness.rebalance_cost = 50.0;  // Placeholder - would calculate actual cost
    }

    return effectiveness;
}

double HedgeManager::calculate_basis_risk(
...
double HedgeManager::convert_dte_to_futures_contract(
    int box_spread_dte,
    InterestRateFutureType future_type) const {

    // Convert box spread DTE to appropriate futures contract
    // This is a simplified mapping - in reality would match to nearest quarterly expiry
