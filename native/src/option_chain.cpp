...
// Many quantitative routines remain stubbed; comments mark where production-grade
// Greeks/IV/DTE calculations should be wired in. Keep data-shaping logic centralized
// in this module so trading components can stay agnostic to quote representation.
...
std::vector<OptionChainEntry> ExpiryChain::filter_by_liquidity(
    ...
    std::copy_if(
        ...
        [min_volume, min_open_interest](const OptionChainEntry& option) {
            return option.meets_liquidity_requirements(min_volume, min_open_interest);
        }
    );
