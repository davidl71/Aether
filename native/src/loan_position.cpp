...
double LoanPosition::get_adjusted_principal() const {
  if (loan_type == LoanType::CPI_LINKED && base_cpi > 0 && current_cpi > 0) {
    // Principal adjusts with CPI: new_principal = original * (current_cpi /
    // base_cpi)
...
double LoanPosition::get_current_interest_rate(double current_shir) const {
  if (loan_type == LoanType::SHIR_BASED) {
    ...
    // If current_shir is provided, use it; otherwise use base interest_rate +
    // spread
    if (current_shir > 0.0) {
...
std::string
time_point_to_iso8601(const std::chrono::system_clock::time_point &tp) {
...
iso8601_to_time_point(const std::string &iso_str) {
