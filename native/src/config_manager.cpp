...
// dependants so downstream modules can assume invariants (non-empty symbols
// list, bounded ROI thresholds, etc.). Extend validation here rather than
// scattering defensive checks across consumers.
...
void ConfigManager::validate_tws_config(const TWSConfig &tws) {
  ...
  if (!tws.connect_options.empty()) {
    bool all_printable =
        std::all_of(tws.connect_options.begin(), tws.connect_options.end(),
                    [](unsigned char c) { return c >= 32 && c < 127; });
    if (!all_printable) {
      throw std::invalid_argument(
          "TWS connect_options must be ASCII printable");
    }
  }
  if (!tws.optional_capabilities.empty()) {
    bool all_printable = std::all_of(
        tws.optional_capabilities.begin(), tws.optional_capabilities.end(),
        [](unsigned char c) { return c >= 32 && c < 127; });
    if (!all_printable) {
      throw std::invalid_argument(
...
void ConfigManager::validate_strategy_params(const StrategyParams &strategy) {
  ...
  if (strategy.min_roi_percent <= 0 || strategy.min_roi_percent > 100) {
    throw std::invalid_argument(
        "Invalid min_roi_percent: " + std::to_string(strategy.min_roi_percent) +
        ". Must be > 0 and <= 100.");
  }
  ...
  if (strategy.max_days_to_expiry < strategy.min_days_to_expiry) {
    throw std::invalid_argument(
        "Invalid DTE range: min=" +
        std::to_string(strategy.min_days_to_expiry) + ", max=" +
        std::to_string(strategy.max_days_to_expiry) + ". Min must be < max.");
  }
...
void ConfigManager::validate_risk_config(const RiskConfig &risk) {
  ...
  if (risk.max_loss_per_position < 0) {
    throw std::invalid_argument(
        "Maximum loss per position must be non-negative");
  }
  ...
  if (risk.position_size_percent <= 0 || risk.position_size_percent > 1) {
    throw std::invalid_argument(
        "Position size percent must be between 0 and 1");
  }
