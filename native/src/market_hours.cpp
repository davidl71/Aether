...
MarketHours::MarketHours() {
  ...
  spdlog::debug(
      "MarketHours initialized with {} holidays and {} early closes for 2025",
...
MarketStatus MarketHours::get_market_status_at(
  ...
  if (is_pre_market_hours(hour_et, minute_et)) {
    ...
    status.reason =
        is_early_close_day ? "regular_hours_early_close" : "regular_hours";
  } else if (is_after_hours(hour_et, minute_et) &&
             hour_et < 20) { // After-hours until 8:00 PM
    status.is_open = true;
...
std::string MarketHours::date_to_string(
    const std::chrono::system_clock::time_point &date) const {
...
std::tuple<int, int, int> MarketHours::get_et_time(
  ...
  if (month == 3) {
    ...
    if (first_sunday == 7)
      first_sunday = 0; // If March 1 is Sunday, first Sunday is March 1
    int second_sunday = first_sunday + 7;

    // DST starts at 2:00 AM on second Sunday
    if (day > second_sunday ||
        (day == second_sunday &&
         hour_utc >= 7)) { // 7 UTC = 2 AM EST (before DST)
      is_dst = true;
    }
  }
  // DST ends: First Sunday in November
    ...
    if (day < first_sunday ||
        (day == first_sunday &&
         hour_utc < 6)) { // 6 UTC = 1 AM EST (after DST ends)
      is_dst = true;
    }
  }
...
bool MarketHours::is_holiday_date(
    const std::chrono::system_clock::time_point &date) const {
  std::string date_str = date_to_string(date);
  return holidays_2025_.count(date_str) > 0;
}

bool MarketHours::is_early_close_date(
    const std::chrono::system_clock::time_point &date) const {
  std::string date_str = date_to_string(date);
...
void MarketHours::update_holiday_calendar(int year) {
  ...
  spdlog::info(
      "Holiday calendar updated for year {} ({} holidays, {} early closes)",
      year, holidays_by_year_[year].size(), early_closes_by_year_[year].size());
}
...
std::string MarketHours::calculate_next_close(
  ...
  if (wday >= 1 && wday <= 5 && !is_holiday_date(from)) {
    ...
    if (hour_et < close_hour || (hour_et == close_hour && minute_et == 0)) {
      ...
      std::ostringstream oss;
      oss << std::setfill('0') << std::setw(4) << (tm.tm_year + 1900) << "-"
          << std::setw(2) << (tm.tm_mon + 1) << "-" << std::setw(2)
          << tm.tm_mday << "T" << std::setw(2) << close_hour << ":00:00-05:00";
      return oss.str();
    }
  }
...
std::set<std::string>
MarketHours::generate_early_closes_for_year(int year) const {
