...
AlpacaStubAdapter::AlpacaStubAdapter(const Config& config)
    ...
    , next_order_id_(1000)
{
    spdlog::info("AlpacaStubAdapter created (STUB MODE - paper_trading={})", config_.paper_trading);
}

AlpacaStubAdapter::~AlpacaStubAdapter() {
    disconnect();
}
...
bool AlpacaStubAdapter::is_connected() const {
    return connected_.load();
}

box_spread::brokers::ConnectionState AlpacaStubAdapter::get_connection_state() const {
...
box_spread::brokers::BrokerCapabilities AlpacaStubAdapter::get_capabilities() const {
...
int AlpacaStubAdapter::request_market_data(
    ...
    if (!connected_.load()) {
        spdlog::warn("AlpacaStubAdapter: Cannot request market data - not connected");
        return -1;
    }
    ...
    spdlog::debug("AlpacaStubAdapter: Market data subscription {} created for {}", request_id, contract.symbol);
    return request_id;
}
...
void AlpacaStubAdapter::cancel_market_data(int request_id) {
    ...
    spdlog::debug("AlpacaStubAdapter: Market data subscription {} cancelled", request_id);
}

std::optional<box_spread::types::MarketData> AlpacaStubAdapter::request_market_data_sync(
    const box_spread::types::OptionContract& contract,
    int timeout_ms
) {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<box_spread::types::OptionContract> AlpacaStubAdapter::request_option_chain(
    ...
    spdlog::debug("AlpacaStubAdapter: Generated mock option chain with {} contracts", chain.size());
    return chain;
}
...
int AlpacaStubAdapter::place_order(
    ...
    spdlog::info("AlpacaStubAdapter: Mock order {} placed (STUB MODE) - {} {} {} @ {}",
...
bool AlpacaStubAdapter::cancel_order(int order_id) {
    ...
    if (it != orders_.end()) {
        ...
        spdlog::info("AlpacaStubAdapter: Mock order {} cancelled (STUB MODE)", order_id);
        return true;
    }

    spdlog::warn("AlpacaStubAdapter: Order {} not found for cancellation", order_id);
    return false;
}

std::optional<box_spread::types::Order> AlpacaStubAdapter::get_order_status(int order_id) const {
    std::lock_guard<std::mutex> lock(mutex_);
...
int AlpacaStubAdapter::place_combo_order(
    ...
    if (contracts.size() != actions.size() || contracts.size() != quantities.size()) {
        spdlog::error("AlpacaStubAdapter: Invalid combo order - mismatched leg counts");
        return -1;
    }
    ...
    spdlog::info("AlpacaStubAdapter: Mock combo order {} placed (STUB MODE) - {} legs",
...
std::optional<box_spread::types::AccountInfo> AlpacaStubAdapter::get_account_info() {
    std::lock_guard<std::mutex> lock(mutex_);
...
box_spread::types::AccountInfo AlpacaStubAdapter::generate_mock_account_info() const {
...
long AlpacaStubAdapter::generate_mock_contract_id(
    ...
    std::hash<std::string> hasher;
    long conid = 1000000 + static_cast<long>(hasher(key) % 9000000);  // 1M-10M range
