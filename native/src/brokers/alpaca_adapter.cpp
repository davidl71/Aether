...
class RateLimiter {
public:
  RateLimiter(int requests_per_minute = 200)
...
AlpacaAdapter::AlpacaAdapter(const Config &config)
    : config_(config), http_client_(std::make_unique<HttpClient>()),
      connection_state_(ConnectionState::Disconnected), next_request_id_(1),
      polling_active_(false) {
  spdlog::info("AlpacaAdapter created (paper_trading={})",
...
static std::string
convert_to_alpaca_symbol(const box_spread::types::OptionContract &contract) {
  ...
  oss << symbol << expiry << right << std::setfill('0') << std::setw(8)
      << strike_cents;
  return oss.str();
}

static box_spread::types::OptionContract
parse_alpaca_symbol(const std::string &alpaca_symbol) {
  // Parse: SPY240119C00450000 -> symbol=SPY, expiry=240119, type=Call,
  // strike=450.0
...
AlpacaAdapter::convert_market_data(const std::string &symbol, double bid,
...
AlpacaAdapter::convert_order(const std::string &order_id,
                             const std::string &symbol,
                             const std::string &status) {
  box_spread::types::Order order;
  order.order_id = std::stoi(order_id);
  // Contract parsing would be needed for full order
  order.status = (status == "filled") ? box_spread::types::OrderStatus::Filled
...
AlpacaAdapter::convert_position(const std::string &symbol, int qty,
                                double avg_price) {
...
std::string AlpacaAdapter::make_request(const std::string &method,
...
bool AlpacaAdapter::is_connected() const { return connected_.load(); }

box_spread::brokers::ConnectionState
AlpacaAdapter::get_connection_state() const {
...
AlpacaAdapter::get_capabilities() const {
...
AlpacaAdapter::request_market_data_sync(
  ...
  try {
    ...
    if (data.contains("quotes") && data["quotes"].is_array() &&
        !data["quotes"].empty()) {
...
AlpacaAdapter::request_option_chain(const std::string &symbol,
    ...
    if (data.contains("contracts") && data["contracts"].is_array()) {
      for (const auto &contract_json : data["contracts"]) {
        ...
        contract.type = (contract_json.value("type", "") == "call")
                            ...
                            : box_spread::types::OptionType::Put;
        contract.style =
            box_spread::types::OptionStyle::American; // Alpaca default
...
long AlpacaAdapter::lookup_contract_id(
    const box_spread::types::OptionContract &contract) {
  std::string alpaca_symbol = convert_to_alpaca_symbol(contract);
...
int AlpacaAdapter::place_order(
  ...
  order["time_in_force"] =
      (tif == box_spread::types::TimeInForce::Day) ? "day" : "gtc";
...
AlpacaAdapter::get_order_status(int order_id) const {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  std::string endpoint = "/v2/orders/" + std::to_string(order_id);
  std::string response =
      const_cast<AlpacaAdapter *>(this)->make_request("GET", endpoint, "");
...
int AlpacaAdapter::place_combo_order(
  ...
  for (size_t i = 0; i < contracts.size(); ++i) {
    ...
    leg["side"] =
        (actions[i] == box_spread::types::OrderAction::Buy) ? "buy" : "sell";
...
AlpacaAdapter::get_position(const box_spread::types::OptionContract &contract) {
...
AlpacaAdapter::get_account_info() {
  std::lock_guard<std::mutex> lock(mutex_);
