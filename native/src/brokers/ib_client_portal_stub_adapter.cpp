// ib_client_portal_stub_adapter.cpp - IB Client Portal stub adapter implementation
...
IBClientPortalStubAdapter::~IBClientPortalStubAdapter() {
    disconnect();
}
...
box_spread::brokers::ConnectionState IBClientPortalStubAdapter::get_connection_state() const {
    return connection_state_.load();
}

box_spread::brokers::BrokerType IBClientPortalStubAdapter::get_broker_type() const {
    return box_spread::brokers::BrokerType::IB_CLIENT_PORTAL;
}

box_spread::brokers::BrokerCapabilities IBClientPortalStubAdapter::get_capabilities() const {
...
int IBClientPortalStubAdapter::request_market_data(
    ...
    if (!connected_.load()) {
        spdlog::warn("IBClientPortalStubAdapter: Cannot request market data - not connected");
        return -1;
    }
    ...
    spdlog::debug("IBClientPortalStubAdapter: Market data subscription {} created for {}", request_id, contract.symbol);
    return request_id;
}
...
void IBClientPortalStubAdapter::cancel_market_data(int request_id) {
    ...
    spdlog::debug("IBClientPortalStubAdapter: Market data subscription {} cancelled", request_id);
}

std::optional<box_spread::types::MarketData> IBClientPortalStubAdapter::request_market_data_sync(
    const box_spread::types::OptionContract& contract,
    int timeout_ms
) {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<box_spread::types::OptionContract> IBClientPortalStubAdapter::request_option_chain(
    ...
    std::vector<double> strikes = {450.0, 460.0, 470.0, 480.0, 490.0, 500.0, 510.0, 520.0, 530.0, 540.0, 550.0};
    ...
    spdlog::debug("IBClientPortalStubAdapter: Generated mock option chain with {} contracts", chain.size());
    return chain;
}
...
int IBClientPortalStubAdapter::place_order(
    ...
    if (!connected_.load()) {
        spdlog::warn("IBClientPortalStubAdapter: Cannot place order - not connected");
        return -1;
    }
    ...
    spdlog::info("IBClientPortalStubAdapter: Mock order {} placed (STUB MODE) - {} {} {} @ {}",
                 ...
                 quantity, contract.symbol, limit_price > 0.0 ? std::to_string(limit_price) : "MARKET");
    return order_id;
}
...
bool IBClientPortalStubAdapter::cancel_order(int order_id) {
    ...
    if (it != orders_.end()) {
        ...
        spdlog::info("IBClientPortalStubAdapter: Mock order {} cancelled (STUB MODE)", order_id);
        return true;
    }

    spdlog::warn("IBClientPortalStubAdapter: Order {} not found for cancellation", order_id);
    return false;
}

std::optional<box_spread::types::Order> IBClientPortalStubAdapter::get_order_status(int order_id) const {
    std::lock_guard<std::mutex> lock(mutex_);
...
int IBClientPortalStubAdapter::place_combo_order(
    ...
    if (!connected_.load()) {
        spdlog::warn("IBClientPortalStubAdapter: Cannot place combo order - not connected");
        return -1;
    }

    if (contracts.size() != actions.size() || contracts.size() != quantities.size()) {
        spdlog::error("IBClientPortalStubAdapter: Invalid combo order - mismatched leg counts");
        return -1;
    }
    ...
    spdlog::info("IBClientPortalStubAdapter: Mock combo order {} placed (STUB MODE) - {} legs",
...
std::vector<box_spread::types::Position> IBClientPortalStubAdapter::get_positions() {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::optional<box_spread::types::Position> IBClientPortalStubAdapter::get_position(
    const box_spread::types::OptionContract& contract
) {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::optional<box_spread::types::AccountInfo> IBClientPortalStubAdapter::get_account_info() {
    std::lock_guard<std::mutex> lock(mutex_);
...
box_spread::types::MarketData IBClientPortalStubAdapter::generate_mock_market_data(
    const box_spread::types::OptionContract& contract
) const {
    ...
    double variation = static_cast<double>(symbol_hash % 100) / 100.0 - 0.5;  // -0.5 to 0.5
    ...
    data.delta = contract.type == box_spread::types::OptionType::Call ? 0.4 : -0.4;
    data.gamma = 0.02;
    data.theta = -0.01;
    data.vega = 0.15;
...
box_spread::types::AccountInfo IBClientPortalStubAdapter::generate_mock_account_info() const {
...
long IBClientPortalStubAdapter::generate_mock_contract_id(
    ...
    std::string key = contract.symbol + contract.expiry +
    ...
    std::hash<std::string> hasher;
    long conid = 20000000 + static_cast<long>(hasher(key) % 80000000);  // 20M-100M range (IBKR format)
