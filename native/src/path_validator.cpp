...
PathValidator::PathValidator(
    const std::vector<std::filesystem::path> &allowed_base_paths)
  ...
  for (auto &path : allowed_base_paths_) {
    try {
      if (std::filesystem::exists(path)) {
        ...
        // Path doesn't exist yet, but we'll allow it (parent directory might
        // exist)
        path = std::filesystem::absolute(path);
        path = path.lexically_normal();
      }
    } catch (const std::filesystem::filesystem_error &e) {
      spdlog::warn("Failed to normalize allowed base path {}: {}",
                   path.string(), e.what());
    }
  }
}
...
PathValidator::validate_path(const std::filesystem::path &file_path,
                             const std::filesystem::path &base_path) const {
  try {
    // Check for directory traversal sequences
    if (contains_traversal(file_path)) {
      spdlog::warn(
          "Path validation failed: contains directory traversal sequences: {}",
          file_path.string());
      return std::nullopt;
    }
    ...
    if (std::filesystem::exists(resolved_path)) {
      ...
      // File doesn't exist yet (e.g., for writing), but we can still validate
      // the parent
      ...
      if (canonical_path.has_parent_path()) {
        ...
        if (std::filesystem::exists(parent)) {
          canonical_path =
              std::filesystem::canonical(parent) / canonical_path.filename();
        }
      }
    }
    ...
    if (!is_within_allowed_boundaries(canonical_path)) {
      spdlog::warn(
          "Path validation failed: path outside allowed boundaries: {}",
    ...
    spdlog::error("Path validation error: {}: {}", file_path.string(),
                  e.what());
    return std::nullopt;
  } catch (const std::exception &e) {
    spdlog::error("Unexpected error during path validation: {}: {}",
...
std::optional<std::filesystem::path>
PathValidator::validate_path(const std::string &file_path_str,
                             const std::filesystem::path &base_path) const {
...
PathValidator create_default_path_validator(
  ...
  if (home) {
    try {
      auto config_dir =
          std::filesystem::path(home) / ".config" / "ib_box_spread";
  ...
  allowed_paths.insert(allowed_paths.end(), additional_paths.begin(),
                       additional_paths.end());
