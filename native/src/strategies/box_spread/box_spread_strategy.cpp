// box_spread_strategy.cpp - Box spread synthetic financing strategy
// implementation
...
// This module encapsulates the decision logic around identifying, validating,
// and executing box spread synthetic financing opportunities (extracting
// risk-free rates for lending/borrowing). It depends on three collaborators:
...
// Production deployments are expected to replace several stubbed sections with
// live integrations; comments flag those extension points to avoid duplicating
// effort in this file.
...
public:
  Impl(tws::TWSClient *client, order::OrderManager *order_mgr,
  ...
  // Local cache of open positions; populated by order callbacks in the full
  // build
...
BoxSpreadStrategy::BoxSpreadStrategy(tws::TWSClient *client,
...
void BoxSpreadStrategy::evaluate_symbol(const std::string &symbol) {
  ...
  pimpl_->stats_.total_opportunities_found +=
      static_cast<int>(opportunities.size());
...
std::vector<BoxSpreadOpportunity>
BoxSpreadStrategy::find_box_spreads(const std::string &symbol) {
  ...
  std::vector<types::OptionContract> contracts =
      pimpl_->client_->request_option_chain(symbol);
  if (contracts.empty()) {
    spdlog::warn("No option contracts found for {}", symbol);
    return {};
  }

  spdlog::debug("Retrieved {} option contracts for {}", contracts.size(),
                symbol);
  ...
  underlying_contract.type =
      types::OptionType::Call; // Dummy, not used for underlying

  double underlying_price = 0.0;
  auto underlying_market_data =
      pimpl_->client_->request_market_data_sync(underlying_contract, 5000);
  if (underlying_market_data.has_value()) {
    ...
    spdlog::warn(
        "Could not get underlying price for {}, will use 0.0 for calculations",
        symbol);
  }
  ...
  for (const auto &contract : contracts) {
    if (contracts_fetched >= max_contracts_to_fetch) {
      spdlog::warn(
          "Limiting market data requests to {} contracts to avoid rate limits",
          max_contracts_to_fetch);
      break;
    }

    std::string contract_key = contract.to_string();
    auto market_data =
        pimpl_->client_->request_market_data_sync(contract, 2000);
  ...
  spdlog::debug(
      "Retrieved market data for {}/{} contracts", market_data_map.size(),
      std::min(static_cast<int>(contracts.size()), max_contracts_to_fetch));

  if (market_data_map.empty()) {
    spdlog::warn("No market data available for any contracts, cannot evaluate "
                 "box spreads");
    return {};
  }
  ...
  option_chain::OptionChain chain =
      option_chain::OptionChainBuilder::build_from_market_data(
  ...
  for (const auto &expiry : chain.get_expiries()) {
    ...
    for (auto &entry : all_options) {
      option_chain::OptionChainBuilder::calculate_metrics(entry,
                                                          underlying_price);
    }
  }
...
std::vector<BoxSpreadOpportunity> BoxSpreadStrategy::find_box_spreads_in_chain(
    const option_chain::OptionChain &chain, double underlying_price) {
  ...
  for (const auto &expiry : expiries) {
    ...
    for (size_t i = 0; i < strikes.size(); ++i) {
      ...
      for (size_t j = i + 1; j < strikes.size(); ++j) {
        ...
        auto long_call_opt =
            expiry_chain.get_option(strike_low, types::OptionType::Call);
        auto short_call_opt =
            expiry_chain.get_option(strike_high, types::OptionType::Call);
        auto long_put_opt =
            expiry_chain.get_option(strike_high, types::OptionType::Put);
        auto short_put_opt =
            expiry_chain.get_option(strike_low, types::OptionType::Put);
        ...
        auto opportunity_opt = evaluate_box_spread(
            long_call_opt->contract, short_call_opt->contract,
            long_put_opt->contract, short_put_opt->contract, chain);
...
std::optional<BoxSpreadOpportunity>
BoxSpreadStrategy::evaluate_box_spread(const types::OptionContract &long_call,
  ...
  auto long_call_entry =
      expiry_chain.get_option(long_call.strike, types::OptionType::Call);
  auto short_call_entry =
      expiry_chain.get_option(short_call.strike, types::OptionType::Call);
  auto long_put_entry =
      expiry_chain.get_option(long_put.strike, types::OptionType::Put);
  auto short_put_entry =
      expiry_chain.get_option(short_put.strike, types::OptionType::Put);
  ...
  // This reveals intraday differences due to bid-ask spreads, put-call parity
  // violations, etc. BUY: Long legs use ASK, short legs use BID
  ...
  if (days_to_expiry > 0 && spread.buy_net_debit > 0) {
    ...
    spread.buy_implied_rate =
        ((spread.buy_net_debit - strike_width) / strike_width) *
  ...
  if (days_to_expiry > 0 && spread.sell_net_credit > 0) {
    ...
    spread.sell_implied_rate =
        ((strike_width - spread.sell_net_credit) / spread.sell_net_credit) *
        (365.0 / days_to_expiry) * 100.0;
  } else {
  ...
  opportunity.liquidity_score =
      (long_call_entry->liquidity_score + short_call_entry->liquidity_score +
       long_put_entry->liquidity_score + short_put_entry->liquidity_score) /
      4.0;

  // Estimate execution probability based on liquidity and spreads
  double avg_spread_pct = (long_call_entry->market_data.get_spread_percent() +
  ...
  opportunity.execution_probability =
      std::min(1.0, (opportunity.liquidity_score / 100.0) *
                        (1.0 - std::min(avg_spread_pct / 10.0, 1.0)));
...
///
/// NOTE: This function is used for synthetic financing rate extraction, not
/// arbitrage. The "profit" represents the basis for calculating implied
/// interest rates.
///
...
///
/// For synthetic financing: This value is used to calculate implied interest
/// rate:
///   implied_rate = ((net_debit - strike_width) / strike_width) × (365 /
///   days_to_expiry) × 100%
///
/// @param spread The box spread leg structure
/// @return Profit basis (theoretical_value - net_debit) used for rate
/// calculation
///
...
double
BoxSpreadStrategy::calculate_roi(const types::BoxSpreadLeg &spread) const {
...
double BoxSpreadStrategy::calculate_confidence_score(
  ...
  double avg_spread =
      (spread.long_call_bid_ask_spread + spread.short_call_bid_ask_spread +
       spread.long_put_bid_ask_spread + spread.short_put_bid_ask_spread) /
      4.0;
  ...
  // How close is net_debit to theoretical value? Efficient markets trade near
  // parity.
  double strike_width = spread.get_strike_width();
  if (strike_width > 0) {
...
bool BoxSpreadStrategy::execute_box_spread(
    const BoxSpreadOpportunity &opportunity) {
  spdlog::info("Executing box spread: profit=${:.2f}, ROI={:.2f}%",
               opportunity.expected_profit, opportunity.spread.roi_percent);
...
bool BoxSpreadStrategy::within_risk_limits(
    const types::BoxSpreadLeg &spread) const {
...
double BoxSpreadStrategy::get_current_exposure() const {
  return std::accumulate(pimpl_->positions_.begin(), pimpl_->positions_.end(),
                         0.0, [](double total, const types::Position &pos) {
...
std::vector<BoxSpreadOpportunity>
BoxSpreadStrategy::find_lending_opportunities(const std::string &symbol,
                                              double benchmark_rate_percent,
                                              double min_spread_bps) {

  spdlog::info("Scanning for lending opportunities for {} vs benchmark rate "
               "{:.2f}% (min spread: {:.1f} bps)",
               symbol, benchmark_rate_percent, min_spread_bps);

  // Find all box spreads for the symbol
  std::vector<BoxSpreadOpportunity> all_opportunities =
      find_box_spreads(symbol);
  ...
  std::sort(lending_opportunities.begin(), lending_opportunities.end(),
            [this, benchmark_rate_percent](const BoxSpreadOpportunity &a,
                                           const BoxSpreadOpportunity &b) {
              double spread_a = BoxSpreadCalculator::compare_to_benchmark(
                  a.spread, benchmark_rate_percent);
              double spread_b = BoxSpreadCalculator::compare_to_benchmark(
                  b.spread, benchmark_rate_percent);
              return spread_a > spread_b; // Higher spread = better
            });

  spdlog::info("Found {} lending opportunities beating benchmark",
               lending_opportunities.size());
  return lending_opportunities;
}

bool BoxSpreadStrategy::beats_benchmark(const types::BoxSpreadLeg &spread,
                                        double benchmark_rate_percent,
                                        double min_spread_bps) const {

  double spread_bps =
      BoxSpreadCalculator::compare_to_benchmark(spread, benchmark_rate_percent);
...
BoxSpreadStrategy::evaluate_position_improvement(
  ...
  if (pimpl_->client_) {
    auto md =
        pimpl_->client_->request_market_data_sync(position.contract, 2000);
    double current_mid =
        md.has_value() ? md->get_mid_price() : position.current_price;
  ...
  // 3. Improvement = current_rate - entry_rate (positive = rates moved in our
  // favor)
  ...
  if (!opportunities.empty()) {
    ...
    double net_roll_benefit_bps =
        (best_rate - improvement.entry_implied_rate) * 100.0 -
        kTransactionCostBps;
  ...
  spdlog::debug(
      "Position {} improvement: current_rate={:.2f}% entry_rate={:.2f}% "
...
bool BoxSpreadStrategy::roll_box_spread(
  ...
  spdlog::info("Rolling box spread position {} to new opportunity",
               position_id);
  ...
  spdlog::info("Successfully rolled position {} to new expiration",
               position_id);
  return true;
}
...
double BoxSpreadStrategy::calculate_early_close_value(
  ...
  // For early close, we would reverse the box spread (sell what we bought, buy
  // what we sold) Early close value = -current_net_debit (opposite of entry
  // cost)
  ...
  double current_market_value =
      spread.theoretical_value; // Would use current market prices
...
void BoxSpreadStrategy::monitor_positions_with_improvements(
  ...
  spdlog::debug("Monitoring positions for improvement opportunities "
                "(threshold: {:.1f} bps)",
  ...
  for (const auto &position : pimpl_->positions_) {
    ...
    if (improvement.improvement_bps < improvement_threshold_bps) {
      spdlog::trace("Position {} has improvement opportunity ({:.1f} bps) but "
                    "below threshold",
    ...
    spdlog::info(
        "Position {} improvement opportunity: {} (improvement: {:.1f} bps)",
        position_id, improvement.improvement_action,
        improvement.improvement_bps);

    // Execute improvement action
    if (improvement.improvement_action == "roll" &&
        improvement.roll_opportunity.has_value()) {
    ...
    } else if (improvement.improvement_action == "close_early" &&
               improvement.early_close_beneficial) {
      spdlog::info("Executing early close for position {} (value: ${:.2f} vs "
                   "hold: ${:.2f})",
                   position_id, improvement.early_close_value,
                   improvement.hold_to_expiry_value);
      close_box_spread(position_id);
    }
  }
}

void BoxSpreadStrategy::update_parameters(
    const config::StrategyParams &params) {
...
types::YieldCurve BoxSpreadStrategy::build_yield_curve(
    ...
    double benchmark_rate_percent, int min_dte, int max_dte) {

  spdlog::info(
      "Building yield curve for {} with strike width {:.2f} (DTE: {}-{})",
  ...
  for (const auto &opp : all_opportunities) {
    ...
    if (std::abs(opp_strike_width - strike_width) <= strike_tolerance &&
      ...
      double implied_rate =
          BoxSpreadCalculator::calculate_implied_interest_rate(opp.spread);
      double effective_rate =
          BoxSpreadCalculator::calculate_effective_interest_rate(opp.spread);
      double spread_bps = BoxSpreadCalculator::compare_to_benchmark(
      ...
      point.expiry_date =
          opp.spread.long_call.expiry; // All legs have same expiry
...
std::vector<types::YieldCurve>
BoxSpreadStrategy::build_yield_curves_multi_symbol(
...
BoxSpreadStrategy::YieldCurveComparison BoxSpreadStrategy::compare_yield_curves(
  ...
  for (const auto &curve : curves) {
    for (int dte : comparison.common_dte_points) {
      ...
      for (const auto &point : curve.points) {
        if (point.days_to_expiry == dte) {
          comparison.rates_by_symbol[curve.symbol].push_back(
              point.implied_rate);
          comparison.spreads_by_symbol[curve.symbol].push_back(
              point.spread_bps);
          break;
        }
      }
...
bool YieldCurvePoint::is_valid() const {
  return !symbol.empty() && days_to_expiry > 0 && strike_width > 0 &&
         !expiry_date.empty();
}

bool YieldCurve::is_valid() const {
  return !symbol.empty() && strike_width > 0 && !points.empty();
}
...
bool BoxSpreadStrategy::YieldCurveComparison::is_valid() const {
  return strike_width > 0 && !symbols.empty() && !common_dte_points.empty();
}
...
bool BoxSpreadValidator::validate_pricing(const types::BoxSpreadLeg &spread) {
  return spread.net_debit > 0 && spread.theoretical_value > 0 &&
         spread.net_debit < spread.theoretical_value;
}

bool BoxSpreadValidator::validate(const types::BoxSpreadLeg &spread,
...
///
/// Mathematical proof:
/// - If S > K2: Long call = (S - K1), Short call = -(S - K2), Long put = 0,
/// Short put = 0
///   Net: (S - K1) - (S - K2) = K2 - K1
/// - If K1 < S <= K2: Long call = (S - K1), Short call = 0, Long put = (K2 -
/// S), Short put = 0
///   Net: (S - K1) + (K2 - S) = K2 - K1
/// - If S <= K1: Long call = 0, Short call = 0, Long put = (K2 - S), Short put
/// = -(K1 - S)
///   Net: (K2 - S) - (K1 - S) = K2 - K1
///
/// @param spread The box spread leg structure
...
///   net_debit = long_call_price - short_call_price + long_put_price -
///   short_put_price
///
/// Explanation:
/// - Long positions (long_call, long_put) cost money (positive contribution)
/// - Short positions (short_call, short_put) generate income (negative
/// contribution)
...
/// @see ALGORITHMS_AND_BEHAVIOR.md for detailed algorithm documentation
double
BoxSpreadCalculator::calculate_net_debit(const types::BoxSpreadLeg &spread) {
...
/// @see ALGORITHMS_AND_BEHAVIOR.md for detailed algorithm documentation
double
BoxSpreadCalculator::calculate_max_profit(const types::BoxSpreadLeg &spread) {

  return spread.theoretical_value - spread.net_debit;
}

double
BoxSpreadCalculator::calculate_max_loss(const types::BoxSpreadLeg &spread) {
...
double
BoxSpreadCalculator::calculate_breakeven(const types::BoxSpreadLeg &spread) {
...
double
BoxSpreadCalculator::calculate_commission(const types::BoxSpreadLeg &spread,
...
double
BoxSpreadCalculator::calculate_total_cost(const types::BoxSpreadLeg &spread,
                                          double per_contract_fee) {
...
double BoxSpreadCalculator::calculate_total_cost_ibkr_pro(
  ...
  return spread.net_debit +
         calculate_commission_ibkr_pro(spread, commission_config);
}
...
///
/// Algorithm: Box spreads can be used as synthetic lending/borrowing
/// instruments. The implied rate represents the annualized interest rate
/// embedded in the spread.
///
/// For Borrowing (net_debit > 0):
///   - We pay net_debit upfront
///   - We receive strike_width at expiration
///   - Formula: rate = ((net_debit - strike_width) / strike_width) * (365 /
///   days_to_expiry) * 100
///
/// For Lending (net_debit < 0, i.e., net_credit > 0):
///   - We receive net_credit upfront
///   - We pay strike_width at expiration
///   - Formula: rate = ((strike_width - net_credit) / net_credit) * (365 /
///   days_to_expiry) * 100
///
/// Expected behavior:
...
double BoxSpreadCalculator::calculate_effective_interest_rate(
    const types::BoxSpreadLeg &spread, double per_contract_fee) {
  ...
  if (total_cost > 0) {
    ...
    effective_rate = ((total_cost - strike_width) / strike_width) *
                     (365.0 / days_to_expiry) * 100.0;
  } else if (total_cost < 0) {
    // Net credit after costs (lending)
    double net_credit_after_costs = -total_cost;
    effective_rate =
        ((strike_width - net_credit_after_costs) / net_credit_after_costs) *
        (365.0 / days_to_expiry) * 100.0;
  }
...
double
BoxSpreadCalculator::compare_to_benchmark(const types::BoxSpreadLeg &spread,
                                          double benchmark_rate_percent,
                                          double per_contract_fee) {

  double effective_rate =
      calculate_effective_interest_rate(spread, per_contract_fee);

  // Return spread in basis points (1 basis point = 0.01%)
  // Positive = box spread beats benchmark (lower rate for borrowing, higher
  // rate for lending) Negative = benchmark beats box spread
...
/// Uses ASK prices for long legs (what we pay) and BID prices for short legs
/// (what we receive). This represents the actual cost to buy the box spread at
/// market prices.
double BoxSpreadCalculator::calculate_buy_net_debit(
    const types::BoxSpreadLeg &spread, double long_call_ask,
    double short_call_bid, double long_put_ask, double short_put_bid) {
...
/// Uses BID prices for long legs (what we receive) and ASK prices for short
/// legs (what we pay). This represents the actual credit received from selling
/// the box spread at market prices.
double BoxSpreadCalculator::calculate_sell_net_credit(
    const types::BoxSpreadLeg &spread, double long_call_bid,
    double short_call_ask, double long_put_bid, double short_put_ask) {
...
double BoxSpreadCalculator::calculate_buy_sell_disparity(double buy_profit,
                                                         double sell_profit) {
...
double BoxSpreadCalculator::calculate_put_call_parity_violation(
    const types::BoxSpreadLeg &spread, double call_implied_rate,
    double put_implied_rate) {

  // Put-call parity violation in basis points
  // Positive = call side implies higher rate, Negative = put side implies
  // higher rate
...
std::vector<BoxSpreadOpportunity>
sort_opportunities_by_profit(std::vector<BoxSpreadOpportunity> opportunities) {
...
std::vector<BoxSpreadOpportunity>
filter_by_min_profit(const std::vector<BoxSpreadOpportunity> &opportunities,
                     double min_profit) {
...
std::vector<BoxSpreadOpportunity>
filter_by_min_roi(const std::vector<BoxSpreadOpportunity> &opportunities,
