...
bool BoxSpreadBag::is_valid() const {
  return !complex_symbol.empty() && !cboe_symbol.empty() && spread.is_valid() &&
         days_to_expiry > 0;
}
...
std::string BoxSpreadBag::generate_cboe_symbol(const std::string &underlying,
  ...
  oss << underlying << " " << expiry << " " << static_cast<int>(strike_low)
      << "/" << static_cast<int>(strike_high) << " BOX";
  return oss.str();
}
...
BoxSpreadBag BoxSpreadBagManager::create_bag_from_spread(
    const types::BoxSpreadLeg &spread, const std::string &underlying_symbol) {
  ...
  bag.cboe_symbol = BoxSpreadBag::generate_cboe_symbol(
      underlying_symbol, spread.long_call.expiry, spread.long_call.strike,
  ...
  bag.implied_rate =
      strategy::BoxSpreadCalculator::calculate_implied_interest_rate(spread);
...
BoxSpreadBag::BagGreeks BoxSpreadBagManager::calculate_bag_greeks(
    ...
    double time_to_expiry, double volatility, double risk_free_rate) {
  ...
  greeks.delta =
      long_call_delta - short_call_delta + long_put_delta - short_put_delta;
  ...
  greeks.gamma =
      gamma_low - gamma_high + gamma_high - gamma_low; // Should cancel

  // Theta (time decay)
  double long_call_theta = option_chain::OptionChainBuilder::calculate_theta(
      underlying_price, strike_low, time_to_expiry_years, volatility,
      risk_free_rate, types::OptionType::Call);
  ...
  double long_put_theta = option_chain::OptionChainBuilder::calculate_theta(
      underlying_price, strike_high, time_to_expiry_years, volatility,
      risk_free_rate, types::OptionType::Put);
  ...
  greeks.theta =
      long_call_theta - short_call_theta + long_put_theta - short_put_theta;
  ...
  greeks.rho = (spread.get_strike_width() - spread.net_debit) *
               time_to_expiry_years * 0.01;

  spdlog::debug("Bag Greeks: delta={:.4f}, gamma={:.4f}, theta={:.4f}, "
                "vega={:.4f}, rho={:.4f}",
...
void BoxSpreadBagManager::update_bag_market_data(BoxSpreadBag &bag, double bid,
                                                 double ask, double last,
                                                 int bid_size, int ask_size) {
...
void BoxSpreadBagManager::update_bag_greeks(BoxSpreadBag &bag,
                                            double underlying_price,
                                            double volatility,
                                            double risk_free_rate) {

  double time_to_expiry = static_cast<double>(bag.days_to_expiry) / 365.0;
  bag.greeks = calculate_bag_greeks(bag.spread, underlying_price,
                                    time_to_expiry, volatility, risk_free_rate);
}

void BoxSpreadBagManager::update_bag_candle(BoxSpreadBag &bag, double price,
                                            double volume) {
