...
// This translation unit glues together the CLI/TUI runtime. It wires
// configuration, logging, component construction, and the long-running trading
// loop. Business logic (pricing, order routing, risk) remains encapsulated in
// their respective modules. When extending behaviour, prefer touching the
// specialized modules first and keep this entry point focused on orchestration
// and lifecycle concerns.

namespace {
std::atomic<bool> g_running{
    true}; // Flipped to false when a termination signal arrives
...
SystemHealth get_system_health(tws::TWSClient *tws_client,
  ...
  if (order_manager) {
    ...
    health.pending_orders =
        order_stats.total_orders_placed - order_stats.total_orders_filled;
  ...
  if (tws_client) {
    ...
    if (rl_status.has_value()) {
      ...
      health.rate_limiter_active_market_data =
          rl_status->active_market_data_lines;
      health.rate_limiter_active_historical =
          rl_status->active_historical_requests;
    } else {
...
void write_snapshot_json(const std::string &output_path,
                         const SystemHealth &health, const config::Config &cfg,
                         strategy::BoxSpreadStrategy *strategy,
                         order::OrderManager *order_manager) {
...
void setup_logging(const config::LogConfig &log_config) {
  try {
    ...
    // File sink (rotating)
    auto max_size =
        static_cast<size_t>(log_config.max_file_size_mb) * 1024 * 1024;
    auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
        log_config.log_file, max_size, log_config.max_files);
    ...
    auto logger = std::make_shared<spdlog::logger>("ib_box_spread",
                                                   sinks.begin(), sinks.end());

    // Set log level
    spdlog::level::level_enum level = spdlog::level::info;
    if (log_config.log_level == "trace")
      level = spdlog::level::trace;
    else if (log_config.log_level == "debug")
    ...
    spdlog::info("Logging initialized: file={}, level={}", log_config.log_file,
...
#ifdef __APPLE__
    return (home / ".config" / "ib_box_spread" / "config.json")
        .lexically_normal();
#else
    return (home / ".config" / "ib_box_spread" / "config.json")
        .lexically_normal();
...
fs::path resolve_config_path(const std::string &requested_path,
                             bool user_override) {
...
#ifdef __APPLE__
      add_candidate(candidates, home / "Library" / "Application Support" /
                                    "ib_box_spread" / "config.json");
#endif
    }

    add_candidate(candidates,
                  fs::path("/usr/local/etc/ib_box_spread/config.json"));
...
void print_config_summary(const config::Config &config) {
  ...
  spdlog::info("  TWS: {}:{} (client_id={})", config.tws.host, config.tws.port,
               config.tws.client_id);
  spdlog::info("  Mode: {}",
               config.dry_run ? "DRY RUN (Paper Trading)" : "LIVE");
...
int main(int argc, char **argv) {
  ...
  auto *config_option =
      app.add_option("-c,--config", config_file,
                     ...
                     "~/.config/ib_box_spread/config.json if present)");
  auto *init_option =
      app.add_option("--init-config", init_config_override,
                     "Write a sample configuration to the provided path "
                     "(default: ~/.config/ib_box_spread/config.json) and exit")
          ->expected(0, 1);
  ...
  if (init_option->count() > 0) {
    ...
    try {
      ...
      std::cerr << "Failed to write sample configuration: " << e.what()
                << std::endl;
      return EXIT_FAILURE;
    }

    std::cout << "Sample configuration written to: " << target.string()
              << std::endl;
  ...
  try {
    config_file =
        resolve_config_path(config_file, user_provided_config).string();
  } catch (const std::exception &e) {
  ...
  try {
    ...
    if (config.tws.use_mock) {
      spdlog::warn("Mock TWS client enabled. No live IBKR connection will be "
                   "attempted.");
    }
  ...
  try {
    ...
    if (!config.broker.priorities.empty()) {
      ...
      priorities = {"mock",   "alpaca", "ib",      "fibi",
                    "meitav", "ibi",    "discount"};
    }
    ...
    for (const auto &broker : priorities) {
      if (broker == "mock") {
        ...
        spdlog::warn(
            "Mock TWS client failed to initialize, trying next broker");
      } else if (broker == "alpaca") {
        spdlog::warn(
            "Alpaca broker selected, but C++ adapter is not implemented. "
        ...
        spdlog::error("Failed to connect to TWS on {}:{}", config.tws.host,
                      config.tws.port);
    ...
    if (!broker_adapter) {
      spdlog::warn(
          "No broker connection established. Falling back to mock client.");
      config.tws.use_mock = true;
      broker_adapter = std::make_unique<brokers::TWSAdapter>(config.tws);
      connected = broker_adapter->connect();
    }
    ...
    if (!connected) {
      spdlog::critical(
          "Unable to initialize any broker client (including mock).");
      if (config.continue_on_error || config.dry_run) {
        spdlog::warn("Proceeding without a live connection. Strategy will "
                     "operate in analysis mode.");
      } else {
        return EXIT_FAILURE;
      }
    }
    ...
    // Commission config: library uses generic CommissionConfig, local uses
    // IBKR-specific Copy basic fields - full conversion would require mapping
    // IBKR tiers to generic tiers
    strategy_params.commissions.per_contract_fee =
        config.strategy.commissions.per_contract_fee;
    strategy_params.commissions.minimum_order_fee =
        config.strategy.commissions.minimum_order_fee;
    strategy_params.commissions.maximum_order_fee_pct =
        config.strategy.commissions.maximum_order_fee_pct;
    strategy_params.commissions.monthly_contract_volume =
        config.strategy.commissions.monthly_contract_volume;
    ...
    auto order_manager = std::make_unique<box_spread::order::OrderManager>(
        broker_adapter.get(), config.dry_run);

    spdlog::info("✓ Order manager initialized (extracted library)");

    // Risk Calculator (extracted library) - constructor may differ, check
    // header
    auto risk_calculator =
        std::make_unique<box_spread::risk::RiskCalculator>(risk_config);
    ...
    auto strategy = std::make_unique<box_spread::strategy::BoxSpreadStrategy>(
        broker_adapter.get(), order_manager.get(), strategy_params);
    ...
    auto order_manager =
        std::make_unique<order::OrderManager>(tws_client.get(), config.dry_run);

    spdlog::info("✓ Order manager initialized");

    // Risk Calculator (local implementation)
    auto risk_calculator = std::make_unique<risk::RiskCalculator>(config.risk);
    ...
    auto strategy = std::make_unique<strategy::BoxSpreadStrategy>(
        tws_client.get(), order_manager.get(), config.strategy);
    ...
    spdlog::info("Monitoring symbols: {}",
                 fmt::join(config.strategy.symbols, ", "));
    ...
    while (g_running) {
      try {
        ...
        if (!broker_adapter || !broker_adapter->is_connected()) {
          // Let the outer loop decide whether to reconnect or shut down based
          // on config
        ...
        // Keeps local state in sync with TWS fills/acks without issuing new
        // orders
        order_manager->update();

        // Execute strategy (evaluate opportunities)
        // Core decision engine: fetches market data snapshots and decides on
        // box spread legs
        ...
        if (++loop_count % 100 == 0) {
          ...
          spdlog::info(
              "Statistics: opportunities={}, trades={}, profit=${:.2f}",
              stats.total_opportunities_found, stats.total_trades_executed,
              stats.total_profit);
          ...
          auto health = get_system_health(
              ...
              order_manager.get(), strategy.get());
#else
          auto health = get_system_health(tws_client.get(), order_manager.get(),
                                          strategy.get());
#endif
          log_system_health(health);

          if (!no_snapshot) {
            write_snapshot_json(snapshot_path, health, config, strategy.get(),
                                order_manager.get());
          }
        }
        ...
        int sleep_time =
            config.loop_delay_ms - static_cast<int>(iteration_duration);
        // Loop cadence is controlled via configuration; avoid busy-waiting when
        // work finishes early
        if (sleep_time > 0) {
    ...
    spdlog::info("  Total opportunities: {}",
                 final_stats.total_opportunities_found);
    ...
    spdlog::info("  Net P&L: ${:.2f}",
                 final_stats.total_profit - final_stats.total_loss);
    if (final_stats.total_trades_executed > 0) {
      spdlog::info("  Win rate: {:.1f}%", final_stats.win_rate * 100.0);
      spdlog::info("  Avg profit/trade: ${:.2f}",
                   final_stats.average_profit_per_trade);
    }
    spdlog::info("═══════════════════════════════════════");
