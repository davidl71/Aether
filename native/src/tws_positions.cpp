...
PositionHandler::PositionHandler(EClientSocket &client) : client_(client) {}
...
std::vector<types::Position>
PositionHandler::request_positions_sync(int timeout_ms,
                                        ConnectionHandler &conn) {
  if (conn.is_mock_mode()) {
    std::lock_guard<std::mutex> lock(position_mutex_);
    return positions_;
...
PositionHandler::get_position(const types::OptionContract &contract) const {
  std::lock_guard<std::mutex> lock(position_mutex_);
...
PositionHandler::request_account_info_sync(int timeout_ms,
                                           ConnectionHandler &conn) {
  if (conn.is_mock_mode()) {
    std::lock_guard<std::mutex> lock(account_mutex_);
...
PositionHandler::query_box_spread_margin(types::BoxSpreadLeg spread,
...
double PositionHandler::get_margin_utilization() const {
  ...
  if (!info || info->initial_margin <= 0.0)
    return 0.0;
...
bool PositionHandler::is_margin_call_risk(double buffer_percent) const {
  auto info = get_account_info();
  if (!info || info->maintenance_margin <= 0.0)
    return false;
...
void PositionHandler::update_mock_positions(
    const types::OptionContract &contract, types::OrderAction action,
...
void PositionHandler::on_position(const std::string &account,
                                  ...
                                  double avg_cost) {
  try {
    spdlog::debug("Position: {} {} @ {} (account={})", position, symbol,
                  avg_cost, account);
    ...
    pos.contract.type =
        (right == "C") ? types::OptionType::Call : types::OptionType::Put;
...
void PositionHandler::on_update_portfolio(
    const std::string &symbol, const std::string &expiry, double strike,
    double position, double market_price, double /*market_value*/,
    double average_cost, double /*unrealized_pnl*/, double /*realized_pnl*/) {
  try {
    std::lock_guard<std::mutex> lock(position_mutex_);
    auto it = std::find_if(positions_.begin(), positions_.end(),
                           [&](const types::Position &candidate) {
                             return candidate.contract.symbol == symbol &&
                                    ...
                                    std::abs(candidate.contract.strike -
                                             strike) < 0.01;
                           });
