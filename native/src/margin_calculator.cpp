...
MarginCalculator::calculate_reg_t_margin(const types::BoxSpreadLeg &spread,
  ...
  double net_margin = short_call_margin + short_put_margin - long_call_premium -
                      long_put_premium;
...
MarginCalculator::calculate_portfolio_margin(const types::BoxSpreadLeg &spread,
  ...
  MarginResult span_result =
      calculate_span_margin(spread, underlying_price, implied_volatility);
  ...
  spdlog::debug(
      "Portfolio margin for box spread: initial=${:.2f}, benefit=${:.2f}",
...
MarginResult
MarginCalculator::calculate_span_margin(const types::BoxSpreadLeg &spread,
                                        double underlying_price,
                                        double implied_volatility) const {
...
MarginResult MarginCalculator::calculate_margin(
    ...
    double implied_volatility, bool prefer_portfolio_margin) const {
...
MarginResult MarginCalculator::calculate_portfolio_margin(
    const std::vector<types::BoxSpreadLeg> &spreads, double underlying_price,
    double implied_volatility) const {
  ...
  for (const auto &spread : spreads) {
    MarginResult margin = calculate_portfolio_margin(spread, underlying_price,
                                                     implied_volatility);
...
bool MarginCalculator::is_margin_call_risk(double current_margin,
  ...
  double maintenance_threshold =
      maintenance_margin * (1.0 + buffer_percent / 100.0);

  return margin_used >= maintenance_threshold ||
         available_margin < maintenance_margin;
}

double
MarginCalculator::calculate_margin_utilization(double margin_used,
...
double MarginCalculator::calculate_remaining_margin_capacity(
    double account_value, double initial_margin_used,
    double maintenance_margin_used) const {

  // Remaining capacity is the difference between account value and maintenance
  // margin
...
double MarginCalculator::calculate_short_option_margin(
    double underlying_price, double strike, types::OptionType type,
    double option_price, double implied_volatility, int days_to_expiry) const {
...
double
MarginCalculator::calculate_long_option_margin(double option_price) const {
...
std::vector<double>
MarginCalculator::calculate_span_scenarios(double underlying_price,
                                           double implied_volatility,
