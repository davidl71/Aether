...
bool ConnectionHandler::connect()
{
  ...
  for (auto& thread : check_threads) { thread.join(); }
  ...
  auto is_paper_port = [](int port) -> bool { return port == 7497 || port == 4002; };
  auto is_live_port = [](int port) -> bool { return port == 7496 || port == 4001; };
  ...
  else if (!open_ports.empty())
  {
    ...
    if (configured_is_paper && !has_paper_port && has_live_port)
    {
      ...
      spdlog::warn("Paper/Live Trading Mismatch: configured paper ({}), only live ports open",
                   config_.port);
    }
    else if (configured_is_live && !has_live_port && has_paper_port)
    {
      paper_live_mismatch = true;
      spdlog::warn("Paper/Live Trading Mismatch: configured live ({}), only paper ports open",
                   config_.port);
    }
  ...
  if (!found_open_port)
  {
    spdlog::error("No open ports found on {}. Checked ports: {}", config_.host, port_list_str);
  ...
  for (int port : open_ports)
  {
    ...
    bool success = client_.eConnect(config_.host.c_str(), port, config_.client_id);
...

    {
      ...
      if (last_error_code_ != 0)
      {
        ...
        if (last_error_code_ == 502 ||
            last_error_code_ == 162 || last_error_code_ == 200)
        {
          client_.eDisconnect();
          continue;
        }
    ...
    if (!client_.isConnected())
    {
      spdlog::warn("Socket not connected after eConnect() for {}:{}", config_.host, port);
      client_.eDisconnect();
      continue;
    }
...
{
  if (mock_mode_) return connected_.load();
...
void ConnectionHandler::on_next_valid_id(int order_id)
{
  try
  {
    ...
    spdlog::info("nextValidId received: {} (connection complete in {}ms)", order_id, total_elapsed);

    next_order_id_ = order_id;

    if (pcap_capture_ && pcap_capture_->is_open())
    {
      std::string event_data = "CONNECTION_COMPLETE:nextValidId=" + std::to_string(order_id);
      std::vector<uint8_t> data(event_data.begin(), event_data.end());
      pcap_capture_->capture_raw(data, false, local_port_, remote_port_);
    }
...
void ConnectionHandler::on_error(int id, int error_code,
                                 ...
                                 const std::string& advanced_order_reject_json)
{
  try
  {
    ...
    {
      spdlog::error("TWS authentication/authorization error {}: {}", error_code, error_string);
      connected_ = false;
      state_ = ConnectionState::Error;
      connection_cv_.notify_all();
    }
    ...
    std::string guidance;
    if (auto it = kIbErrorGuidance.find(error_code); it != kIbErrorGuidance.end())
    {
      guidance = it->second;
    }
    ...
    if (error_code >= 2100 && error_code < 3000)
    {
      ...
      if (!guidance.empty()) spdlog::info("  -> {}", guidance);
    }
    else if (error_code >= 1100 && error_code < 2000)
    {
      spdlog::warn("[IB System {}] ID: {} | {}", error_code, id, error_string);
      if (!guidance.empty()) spdlog::warn("  -> {}", guidance);

      if (error_code == 1100)
      {
        connected_ = false;
        state_ = ConnectionState::Error;
        if (config_.auto_reconnect) attempt_reconnect_with_backoff();
      }
    ...
    else
    {
      ...
      if (!guidance.empty()) spdlog::error("  -> {}", guidance);
    }

    if (!guidance.empty()) guidance_notes.emplace_back(guidance);

    for (size_t i = 0; i < kErrorPhraseGuidanceCount; ++i)
    {
      if (error_string.find(kErrorPhraseGuidance[i].first) != std::string::npos)
      {
...
void ConnectionHandler::configure_rate_limiter(const RateLimiterConfig& rl_config)
{
  rate_limiter_.configure(rl_config);
}

std::optional<RateLimiterStatus> ConnectionHandler::get_rate_limiter_status() const
{
  if (!rate_limiter_.is_enabled()) return std::nullopt;
  return rate_limiter_.get_status();
}

void ConnectionHandler::cleanup_stale_rate_limiter_requests(std::chrono::seconds max_age)
{
...
std::chrono::system_clock::time_point ConnectionHandler::get_server_time() const
{
...
void ConnectionHandler::start_reader_thread()
{
  ...
  reader_thread_ = std::make_unique<std::thread>([this, r = std::move(reader)]() mutable {
    ...
    while (client_.isConnected() || connected_.load())
    {
      ...
      if (!client_.isConnected() && !connected_.load()) break;
    ...
    spdlog::debug("EReader thread stopped after {}s ({} messages)", uptime, message_count);
  });
}
...
void ConnectionHandler::attempt_reconnect_with_backoff()
{
  ...
  const int max_retries = config_.max_reconnect_attempts > 0
      ? config_.max_reconnect_attempts : 10;
  ...
  std::thread([this]() {
    if (connect())
    {
      spdlog::info("Reconnected successfully after {} attempts", reconnect_attempts_.load());
    }
  }).detach();
}
...
{
  if (health_check_enabled_.load()) return;
  ...
  health_check_thread_ = std::make_unique<std::thread>([this]() {
    ...
    while (health_check_enabled_.load() && connected_.load())
    {
      ...
      {
        spdlog::warn("Connection appears stale ({}s since last message)",
                     std::chrono::duration_cast<std::chrono::seconds>(time_since_last).count());
...
bool ConnectionHandler::wait_for_connection_with_progress(int timeout_ms)
{
  ...
  while (true)
  {
...

    {
      ...
      {
        spdlog::warn("Connection error {} during wait: {}", last_error_code_, last_error_message_);
        return false;
      }
    }
    ...
    {
      int progress = 0;
      if (connection_callbacks_received_.connectAck) progress += 33;
      if (connection_callbacks_received_.managedAccounts) progress += 33;
      spdlog::info("Connection progress: {}% ({}ms remaining)", progress, timeout_ms - elapsed);
      last_log = now;
    }
