...
PcapCapture::PcapCapture(const std::string &output_file,
                         bool use_nanosecond_precision)
    : output_file_(output_file),
      use_nanosecond_precision_(use_nanosecond_precision), is_open_(false) {
  stats_.packets_captured = 0;
  stats_.bytes_captured = 0;
}

PcapCapture::~PcapCapture() { close(); }
...
void PcapCapture::close() {
  ...
  spdlog::info("PCAP capture closed: {} ({} packets, {} bytes)", output_file_,
               stats_.packets_captured, stats_.bytes_captured);
}

bool PcapCapture::write_global_header() {
  PcapGlobalHeader header;

  // Magic number: 0xa1b2c3d4 for microsecond precision, 0xa1b23c4d for
  // nanosecond
...
bool PcapCapture::write_packet(
  ...
  // Write packet data
  file_.write(reinterpret_cast<const char *>(packet_data.data()),
              packet_data.size());
...
bool PcapCapture::capture_packet(uint32_t src_ip, uint32_t dst_ip,
                                 uint16_t src_port, uint16_t dst_port,
                                 const std::vector<uint8_t> &data,
                                 bool direction) {
  return capture_packet(src_ip, dst_ip, src_port, dst_port, data,
                        get_timestamp(), direction);
}

bool PcapCapture::capture_packet(
    uint32_t src_ip, uint32_t dst_ip, uint16_t src_port, uint16_t dst_port,
    const std::vector<uint8_t> &data,
    const std::chrono::system_clock::time_point &timestamp, bool direction) {
  if (!is_open_) {
    return false;
  }
...
bool PcapCapture::capture_raw(const std::vector<uint8_t> &data, bool direction,
                              uint16_t src_port, uint16_t dst_port) {
  if (!is_open_) {
    return false;
  }
  ...
  return capture_packet(localhost_ip, localhost_ip, src_port, dst_port, data,
                        direction);
}
...
std::vector<uint8_t>
PcapCapture::create_localhost_frame(uint32_t src_ip, uint32_t dst_ip,
                                    uint16_t src_port, uint16_t dst_port,
...
uint16_t PcapCapture::calculate_tcp_checksum(uint32_t src_ip, uint32_t dst_ip,
...
// Convert to network byte order (host to network long)
  ...
  return ((result & 0xff000000) >> 24) | ((result & 0x00ff0000) >> 8) |
...
std::string generate_pcap_filename(const std::string &prefix) {
  ...
  oss << prefix << "_"
      ...
      << std::setfill('0') << std::setw(3) << ms.count() << ".pcap";
  return oss.str();
}
