...
ContractHandler::ContractHandler(EClientSocket &client) : client_(client) {}
...
int ContractHandler::request_contract_details(
    const types::OptionContract &contract, ContractDetailsCallback callback,
  ...
  if (!conn.check_rate_limit()) {
    spdlog::error("Rate limit exceeded for contract details: {}",
                  contract.to_string());
    return -1;
  }
...
void ContractHandler::on_contract_details(int req_id, long con_id) {
  try {
    spdlog::debug("Contract details received: reqId={}, conId={}", req_id,
                  con_id);
    ...
    if (callback)
      callback(con_id);
    if (promise)
      promise->set_value(con_id);
  } catch (const std::exception &e) {
    spdlog::error("Exception in contractDetails(reqId={}): {}", req_id,
                  e.what());
  }
}
...
void ContractHandler::on_contract_details_end(int req_id) {
  try {
    ...
    if (callbacks_.count(req_id)) {
      spdlog::warn("Contract details end without result for async reqId={}",
                   req_id);
      callbacks_.erase(req_id);
    }
    results_.erase(req_id);
  } catch (const std::exception &e) {
    spdlog::error("Exception in contractDetailsEnd(reqId={}): {}", req_id,
                  e.what());
  }
}
