...
bool LoanManager::initialize(const std::string& loans_file_path) {
    ...
    if (!load_from_json()) {
        spdlog::warn("Could not load loans from {}, starting with empty loan list", loans_file_path);
...
bool LoanManager::update_loan(const std::string& loan_id, const LoanPosition& loan) {
    if (!validate_loan(loan)) {
...
std::optional<LoanPosition> LoanManager::get_loan(const std::string& loan_id) const {
...
bool LoanManager::update_cpi_for_all_loans(double current_cpi) {
    ...
    for (auto& [loan_id, loan] : loans_) {
        if (loan.loan_type == LoanType::CPI_LINKED && loan.status == LoanStatus::ACTIVE) {
...
bool LoanManager::update_shir_for_all_loans(double current_shir) {
    ...
    int updated = 0;
    for (auto& [loan_id, loan] : loans_) {
        if (loan.loan_type == LoanType::SHIR_BASED && loan.status == LoanStatus::ACTIVE) {
...
bool LoanManager::save_to_json() const {
    ...
    try {
        ...
        for (const auto& [loan_id, loan] : loans_) {
            ...
            loan_json["origination_date"] = time_point_to_iso8601(loan.origination_date);
            loan_json["maturity_date"] = time_point_to_iso8601(loan.maturity_date);
            loan_json["next_payment_date"] = time_point_to_iso8601(loan.next_payment_date);
        ...
        if (!file.is_open()) {
            spdlog::error("Failed to open loans file for writing: {}", loans_file_path_);
            return false;
        }
    ...
    try {
        ...
        if (j.contains("loans") && j["loans"].is_array()) {
            for (const auto& loan_json : j["loans"]) {
                ...
                loan.origination_date = iso8601_to_time_point(loan_json["origination_date"]);
                loan.maturity_date = iso8601_to_time_point(loan_json["maturity_date"]);
                loan.next_payment_date = iso8601_to_time_point(loan_json["next_payment_date"]);
