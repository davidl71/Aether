// cache_client_memcached.cpp - Memcached backend for CacheClient (optional, requires libmemcached)
...
public:
  MemcachedClient(const std::string& host, int port, const std::string& prefix, int default_ttl);
    ...
    : prefix_(prefix), default_ttl_(default_ttl_ > 0 ? default_ttl_ : 300)
{
  ...
  memcached_server_st* servers = memcached_server_list_append(nullptr, host.c_str(), port, nullptr);
  if (!servers)
  {
...
void MemcachedClient::set(const std::string& key, const std::string& value, int ttl_seconds)
{
  std::lock_guard<std::mutex> lock(mutex_);
  if (!memcached_)
    return;
  ...
  std::string full_key = make_key(key);
  memcached_set(memcached_, full_key.c_str(), full_key.size(),
                value.data(), value.size(), static_cast<time_t>(ttl), 0);
}
