...
std::optional<types::MarketData> market_data_from_string(const std::string &s) {
  ...
  std::string token;
  auto next = [&]() -> bool {
    return static_cast<bool>(std::getline(is, token, '\t'));
  };
  if (!next())
    return std::nullopt;
  md.symbol = token;
  ...
  md.open = read_d();
  if (next() && token != "n")
    md.implied_volatility = std::stod(token);
  if (next() && token != "n")
    md.delta = std::stod(token);
  if (next() && token != "n")
    md.gamma = std::stod(token);
  if (next() && token != "n")
    md.theta = std::stod(token);
  if (next() && token != "n")
    md.vega = std::stod(token);
  return md;
}

} // namespace

MarketDataHandler::MarketDataHandler(EClientSocket &client) : client_(client) {}

void MarketDataHandler::set_market_data_cache(platform::CacheClient *cache,
                                              int ttl_seconds) {
...
int MarketDataHandler::request_market_data(
    const types::OptionContract &contract, MarketDataCallback callback,
  ...
  if (!conn.can_start_market_data(request_id)) {
    spdlog::error("Market data line limit exceeded for {}",
                  contract.to_string());
    return -1;
  }
  ...
  client_.reqMktData(request_id, tws_contract, "", false, false,
                     TagValueListSPtr());
...
std::optional<types::MarketData> MarketDataHandler::request_market_data_sync(
  ...
  if (market_data_cache_ && market_data_cache_->is_healthy()) {
    ...
    if (opt.has_value()) {
      ...
      if (parsed.has_value() &&
          (parsed->bid > 0 || parsed->ask > 0 || parsed->last > 0)) {
  ...
  if (!conn.check_rate_limit()) {
    spdlog::error("Rate limit exceeded for sync market data: {}",
                  contract.to_string());
    return std::nullopt;
  }
  ...
  if (future.wait_for(std::chrono::milliseconds(timeout_ms)) ==
    ...
    if (data.bid > 0 || data.ask > 0 || data.last > 0) {
      if (market_data_cache_ && market_data_cache_->is_healthy())
        market_data_cache_->set(cache_key, market_data_to_string(data),
                                market_data_cache_ttl_);
      return data;
    }
  } else {
...
void MarketDataHandler::cancel_market_data(int request_id,
                                           ConnectionHandler &conn) {
  if (conn.is_mock_mode()) {
    std::lock_guard<std::mutex> lock(mutex_);
...
MarketDataHandler::request_option_chain(const std::string &symbol,
  ...
  if (underlying_future.wait_for(std::chrono::milliseconds(3000)) ==
      std::future_status::ready) {
  ...
  spdlog::info("Retrieved {} option contracts for {}", contracts.size(),
               symbol);
  return contracts;
}
...
void MarketDataHandler::on_tick_price(int ticker_id, int field, double price) {
  try {
    ...
    spdlog::error("Exception in tickPrice(id={}, field={}): {}", ticker_id,
                  field, e.what());
  }
}
...
void MarketDataHandler::on_tick_size(int ticker_id, int field, int64_t size) {
  try {
    ...
    switch (field) {
    case 0:
      md.bid_size = static_cast<int>(size);
      break; // BID_SIZE
    case 3:
      md.ask_size = static_cast<int>(size);
      break; // ASK_SIZE
    case 5:
      md.last_size = static_cast<int>(size);
      break; // LAST_SIZE
    case 8:
      md.volume = static_cast<double>(size);
      break; // VOLUME
    default:
      break;
    }
  } catch (const std::exception &e) {
    spdlog::error("Exception in tickSize(id={}, field={}): {}", ticker_id,
                  field, e.what());
  }
}

void MarketDataHandler::on_tick_option_computation(int ticker_id, int /*field*/,
                                                   double implied_vol,
  ...
  try {
    ...
    spdlog::error("Exception in tickOptionComputation(id={}): {}", ticker_id,
                  e.what());
  }
}

void MarketDataHandler::on_security_definition_optional_parameter(
    int req_id, const std::string &exchange, int /*underlying_con_id*/,
    const std::string & /*trading_class*/, const std::string & /*multiplier*/,
  ...
  try {
    ...
    std::lock_guard<std::mutex> lock(option_chain_mutex_);
    if (option_chain_promises_.count(req_id) == 0)
      return;

    option_chain_expirations_[req_id].insert(expirations.begin(),
                                             expirations.end());
    option_chain_strikes_[req_id].insert(strikes.begin(), strikes.end());
  } catch (const std::exception &e) {
    spdlog::error("Exception in secDefOptParam(reqId={}): {}", req_id,
                  e.what());
  }
}

void MarketDataHandler::on_security_definition_optional_parameter_end(
    int req_id) {
  try {
    ...
    spdlog::info("Option chain complete: {} contracts for {}", contracts.size(),
                 symbol);
    if (option_chain_promises_.count(req_id)) {
      option_chain_promises_[req_id]->set_value(contracts);
    }
  } catch (const std::exception &e) {
    spdlog::error("Exception in secDefOptParamEnd(reqId={}): {}", req_id,
                  e.what());
    std::lock_guard<std::mutex> lock(option_chain_mutex_);
    if (option_chain_promises_.count(req_id)) {
      option_chain_promises_[req_id]->set_value(
          std::vector<types::OptionContract>());
    }
  }
}
