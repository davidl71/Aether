...
OrderHandler::OrderHandler(EClientSocket &client) : client_(client) {}
...
int OrderHandler::place_combo_order(
  ...
  spdlog::info("Placed combo order #{}: {} legs, total limit: {:.2f}", order_id,
               contracts.size(), total_limit);
...
void OrderHandler::on_order_status(int order_id, const std::string &status,
  ...
  try {
    ...
    spdlog::error("Exception in orderStatus(orderId={}): {}", order_id,
                  e.what());
  }
}
...
void OrderHandler::on_exec_details(int /*req_id*/, int order_id, double shares,
                                   double price) {
  try {
    spdlog::info("Execution: order={}, shares={}, price={}", order_id, shares,
                 price);
