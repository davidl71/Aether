...
double ConvexityCalculator::calculate_portfolio_convexity(
    ...
    double short_term_convexity,
    double long_term_weight,
    double long_term_convexity_value) const {
...
double ConvexityCalculator::calculate_current_convexity(
    ...
    double long_term_weight,
    const BondData& long_term_bond) const {
...
bool ConvexityCalculator::should_rebalance(
    ...
    double deviation_percent = std::abs((current_convexity - target_convexity) / target_convexity) * 100.0;
...
double convexity_objective(unsigned n, const double* x, double* grad, void* data) {
    ...
    // Portfolio convexity = weighted average
    double portfolio_convexity = x[0] * short_bond.convexity + x[1] * long_bond.convexity;
...
double duration_constraint(unsigned n, const double* x, double* grad, void* data) {
    auto* constraint_data = static_cast<std::pair<std::pair<BondData, BondData>, double>*>(data);
    ...
    double portfolio_duration = x[0] * short_bond.duration + x[1] * long_bond.duration;
...
OptimizationResult ConvexityCalculator::optimize_barbell_allocation(
    ...
    try {
        ...
        std::pair<std::pair<BondData, BondData>, double> duration_data(
            std::make_pair(short_term_bond, long_term_bond),
            target_duration
        );
        ...
        if (opt_result < 0) {
            result.error_message = "Optimization failed with code: " + std::to_string(opt_result);
        ...
        result.portfolio_convexity = -minf;  // Negative because we minimized negative
        result.portfolio_duration = calculate_weighted_duration(
            result.short_term_weight,
            short_term_bond.duration,
            result.long_term_weight,
            long_term_bond.duration
        );
        result.success = true;

        spdlog::debug("Convexity optimization: short_weight={:.4f}, long_weight={:.4f}, "
