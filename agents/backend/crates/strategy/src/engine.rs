  ...
  pub async fn process_signals(&mut self) -> anyhow::Result<usize> {
    ...
    while let Ok(signal) = self.event_rx.try_recv() {
      ...
      let decision_opt = Python::with_gil(|py| -> anyhow::Result<Option<Decision>> {
        ...
        let result = self.py_strategy.call_method_bound(py, "on_signal", (), Some(&kwargs));
        ...
        match result {
          Ok(py_result) => {
            ...
            let side_str: String = py_result.getattr(py, "side")
              .and_then(|v| v.extract(py))
              .unwrap_or_else(|_| "buy".to_string());
            let quantity: i32 = py_result.getattr(py, "quantity")
              .and_then(|v| v.extract(py))
              .unwrap_or(0);
            let symbol: String = py_result.getattr(py, "symbol")
              .and_then(|v| v.extract(py))
            ...
            Ok(Some(Decision { symbol, quantity, side }))
          }
          Err(e) => {
