...
pub fn render_settings(f: &mut Frame, app: &App, area: Rect) {
    ...
    let component_count = app.backend_health.len();
    ...
    let flow_lines = vec![
        ...
        Line::from(vec![Span::raw("  +- tws_yield_curve_daemon "), {
            let yw_entry = app
                .backend_health
    ...
    if app.settings_add_symbol_input.is_some() && app.settings_edit_config_key.is_none() {
        ...
        f.render_widget(
            Paragraph::new(prompt_lines).block(symbols_block),
            symbols_area,
        );
...
pub(crate) fn render_settings_health_section(f: &mut Frame, app: &App, area: Rect) {
    ...
    let component_lines = settings_component_lines(app);
    let health_chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([Constraint::Min(6), Constraint::Length(4)])
        .split(area);
...
pub(crate) fn render_settings_sources_section(f: &mut Frame, app: &App, area: Rect) {
    let sources_block = section_block("Data sources", section_active(app, 3));
...
fn settings_health_flow_lines(
    ...
    vec![
        Line::from(vec![
            ...
            Span::styled(nats_label, Style::default().fg(nats_color)),
            Span::raw("  components: "),
            Span::styled(component_count.to_string(), Style::default().fg(Color::Cyan)),
        ]),
        Line::from(vec![
            Span::raw("  +- "),
            Span::styled(be_sym, Style::default().fg(be_color)),
...
#[allow(dead_code)]
fn settings_component_lines(app: &App) -> Vec<Line<'static>> {
    let mut component_names: Vec<_> = app.backend_health.keys().cloned().collect();
    component_names.sort();
