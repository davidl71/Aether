...
fn render_status_bar(f: &mut Frame, app: &App, area: Rect) {
    ...
    if let Some(ref snap) = app.snapshot {
        let aid = snap.inner.account_id.trim();
        if !aid.is_empty() {
            spans.push(Span::raw(" | "));
    ...
    if app.nats_status.state != ConnectionState::Connected
        && !app.nats_status.detail.is_empty()
    {
        spans.push(Span::raw(" "));
        spans.push(Span::styled(
...
fn render_main(f: &mut Frame, app: &App, area: Rect) {
    if app.split_pane {
        ...
        match app.active_tab {
            ...
            Tab::Yield => yield_curve::render_yield_curve(f, app, area),
    ...
    let (title, lines) = match content {
        DetailPopupContent::Order(o) => (
            ...
            vec![
                ...
                Line::from(format!("Side:     {}", o.side)),
                Line::from(format!("Qty:      {}", o.quantity)),
                Line::from(format!("Status:   {}", o.status)),
                Line::from(format!("Submitted: {}", o.submitted_at.format("%Y-%m-%d %H:%M:%S UTC"))),
            ],
        ),
        DetailPopupContent::Position(p) => (
            ...
            vec![
                ...
                Line::from(format!("Account:  {}", p.account_id.as_deref().unwrap_or("—"))),
            ],
        ),
...
fn render_help_overlay(f: &mut Frame, area: Rect) {
    let lines = vec![
        ...
        Line::from(vec![
            ...
            Span::raw("jump to Dash / Pos / Orders / Alerts / Yield / Scen / Logs / Set"),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::raw("Scen: detail  "),
            Span::styled(" + − ", Style::default().add_modifier(Modifier::BOLD)),
            Span::raw("Logs level"),
        ]),
        Line::from(vec![
            Span::styled(" ← → ", Style::default().add_modifier(Modifier::BOLD)),
            Span::raw("Yield: change symbol"),
        ]),
        Line::from(""),
        Line::from(Span::styled(
...
fn render_hint_bar(f: &mut Frame, app: &App, area: Rect) {
    let mut spans = vec![
        ...
        Span::raw(if app.split_pane { ":single pane" } else { ":split pane" }),
    ];
    ...
    if app.active_tab == Tab::Settings {
        ...
        spans.push(Span::styled("Del", Style::default().add_modifier(Modifier::BOLD)));
