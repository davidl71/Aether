...
pub fn spawn_health_aggregator(state: SharedHealthAggregate, nats_url: Option<String>) {
    ...
    let factory: crate::supervisor::TaskFactory = Arc::new(move || {
        let state = state.clone();
        let nats_url = nats_url.clone();
        async move { run_loop(state, nats_url).await }.boxed()
    });

    crate::supervisor::spawn_supervised("health_aggregation", factory);
}

async fn run_loop(state: SharedHealthAggregate, nats_url: String) -> anyhow::Result<()> {
                    ...
                    Ok(mut subscriber) => {
                        while let Some(message) = subscriber.next().await {
                            ...
                            let mut transport = health
                                ...
                                .with_subject(topics::system::health())
                                .with_role("subscriber");
                            transport.reconnect_count = reconnect_cycles.load(Ordering::Relaxed);
