...
async fn run_fmp(nc: Client, fmp: Arc<FmpClient>) {
    let sub_income = match nc.subscribe(SUBJECT_FMP_INCOME_STATEMENT.to_string()).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let sub_balance = match nc.subscribe(SUBJECT_FMP_BALANCE_SHEET.to_string()).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let sub_cash = match nc.subscribe(SUBJECT_FMP_CASH_FLOW.to_string()).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let sub_quote = match nc.subscribe(SUBJECT_FMP_QUOTE.to_string()).await {
        Ok(s) => s,
        Err(e) => {
