...
use nats_adapter::{ChannelBridge, NatsClient, topics};
...
async fn test_wildcard_subscriptions() {
  ...
  let publisher_spy = bridge.create_publisher(
    topics::market_data::tick("SPY"),
    "test",
    "MarketDataTick",
  );
  let publisher_xsp = bridge.create_publisher(
  ...
  publisher_spy.publish(tick_spy).await.expect("Failed to publish");
  publisher_xsp.publish(tick_xsp).await.expect("Failed to publish");
  ...
  assert!(received >= 2, "Expected at least 2 messages, got {}", received);
}
